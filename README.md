...
- 导出 `*_result.csv`
- 已重构为 DJI 风格暗色中控台 UI：左侧快捷栏、中央视频画布、右侧状态/标注卡片、底部回放控制台
...
- 左侧：DJI 风格竖向快捷栏，提供打开视频、保存/读取标注、导出 AVI、导出 CSV、退出等入口
- 中央：`188 x 120` 视频自适应放大显示，叠加检测圆、圆心、编号和纠错图形
- 右侧：飞控状态/当前帧信息、检测结果、算法信息、错误标注面板
- 底部：回放控制台，包含播放、暂停、上一帧、下一帧、原始/二值视图、进度条、跳转帧、跳转时间
- 顶部：保留菜单栏，承载文件、视图、帮助等传统桌面入口
...
- 鼠标悬浮在视频画面内时，右侧“飞控状态 / 当前帧”和底部状态栏会显示原始像素 `X/Y/Gray` 亮度值，便于判断二值化阈值。
