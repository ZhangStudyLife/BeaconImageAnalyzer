...
#define BEACON_SIDE_EDGE_MIN_PEAK       150
...
int g_image_down_beacon_binary_threshold = 120;
...
#define CAR_LAMP_SIDE_SUN_EDGE_MARGIN   36
...
static int component_peak_gray(const component_t *comp)
{
    int x;
    int y;
    int peak = 0;
    ...
    for (y = comp->min_y; y <= comp->max_y; y++)
    {
        ...
        {
            if (g_current_image[y][x] > peak)
            {
                peak = g_current_image[y][x];
            }
        }
    }
...
static unsigned char is_lamp_candidate(const component_t *comp)
{
    ...
    if ((comp->min_x <= CAR_LAMP_SIDE_SUN_MARGIN ||
        ...
        (comp->cy < CAR_LAMP_SIDE_SUN_EDGE_MARGIN ||
         comp->cy >= BEACON_IMAGE_H - CAR_LAMP_SIDE_SUN_EDGE_MARGIN) &&
        local_background_average(comp, CAR_LAMP_LOCAL_RING_PAD) >
...
static unsigned char is_close_lamp_core(const component_t *comp)
{
    ...
    bbox_w = comp->max_x - comp->min_x + 1;
    bbox_h = comp->max_y - comp->min_y + 1;
    bbox_long = bbox_w > bbox_h ? bbox_w : bbox_h;
    bbox_short = bbox_w < bbox_h ? bbox_w : bbox_h;
    return (comp->area >= CLOSE_LAMP_CORE_MIN_AREA &&
            bbox_long >= CLOSE_LAMP_CORE_MIN_BBOX_W &&
            bbox_short <= CLOSE_LAMP_CORE_MAX_BBOX_H) ? 1 : 0;
}
...
static unsigned char make_half_lamp_core(
    const component_t *merged_core,
    unsigned char split_x,
    unsigned char take_second_half,
    component_t *lamp_core)
{
    ...
    split = split_x != 0
        ? (merged_core->min_x + merged_core->max_x + 1) / 2
        : (merged_core->min_y + merged_core->max_y + 1) / 2;
    lamp_core->min_x = BEACON_IMAGE_W - 1;
    lamp_core->min_y = BEACON_IMAGE_H - 1;

    for (y = merged_core->min_y; y <= merged_core->max_y; y++)
    {
        for (x = merged_core->min_x; x <= merged_core->max_x; x++)
        {
            int coordinate = split_x != 0 ? x : y;

            if ((take_second_half == 0 && coordinate >= split) ||
                (take_second_half != 0 && coordinate < split))
            {
                continue;
            }
...
static unsigned char make_symmetric_lamp_core(
    const component_t *merged_core,
    component_t *best_lamp)
{
    ...
    for (split_x = 0; split_x <= 1; split_x++)
    {
        for (take_second_half = 0; take_second_half <= 1; take_second_half++)
        {
            component_t candidate;
            float score;

            if (make_half_lamp_core(merged_core, split_x,
                                    take_second_half, &candidate) == 0)
            {
                continue;
            }

            score = lamp_score(&candidate);
            if (found == 0 || score > best_score)
            {
                *best_lamp = candidate;
                best_score = score;
    ...
    component_t *best_lamp)
{
    ...
    if (has_merged_core != 0 && make_symmetric_lamp_core(&merged_core, best_lamp) != 0)
    {
        return 1;
    }
    ...
    beacon_result_t *result)
{
    ...
    if (is_side_edge_beacon(comp) != 0 &&
        component_peak_gray(comp) < BEACON_SIDE_EDGE_MIN_PEAK)
    {
        return;
    }
...
{
    (void)image;
    return (unsigned char)g_image_down_beacon_binary_threshold;
}
