...
{
    beacon_circle_t *beacon = &result->temporal_beacons[0];
...
{
    beacon_rect_t *car = &result->temporal_car_lamps[0];
...
static unsigned char update_temporal_car(beacon_result_t *result)
{
    ...
    if (measurement == 0)
    {
        return predict_missed_track(&g_car_track, CAR_LAMP_MAX_MISSES);
    }
