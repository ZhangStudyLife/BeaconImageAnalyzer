...
static const float s_maximums[] = {
    255.0f, 636.0f, 4.0f, 22560.0f, 22560.0f, 255.0f,
...
uint8 image_param_info(uint16 index,
                       ...
                       float *maximum)
{
    if (index >= image_param_count() || id == 0 || type == 0 ||
        minimum == 0 || maximum == 0)
    {
...
{
    return image_down_remote_param_execute(IPC_REMOTE_PARAM_OP_GET,
                                           type, id, 0U, actual_bits);
}
...
uint8 image_param_set(uint8 type,
                      ...
                      uint32 *actual_bits)
{
    return image_down_remote_param_execute(IPC_REMOTE_PARAM_OP_SET,
                                           type, id, value_bits, actual_bits);
}
