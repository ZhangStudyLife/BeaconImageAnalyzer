...
#define IMAGE_MEMORY_BARRIER()    __DMB()
#else
#define IMAGE_MEMORY_BARRIER()    ((void)0)
#endif
...
#endif
#ifndef GRAY_BEACON_DIRECT_MAX_BRIGHT_PIXELS
#define GRAY_BEACON_DIRECT_MAX_BRIGHT_PIXELS 512U
#endif
#define GRAY_BEACON_MIN_PEAK_GRAY      60U
#define GRAY_WEAK_MIN_PEAK_GRAY        25U
#define GRAY_WEAK_MAX_PEAKS             8
#define GRAY_BEACON_PEAK_MERGE_DISTANCE_SQ 25
#define GRAY_WEAK_MAX_Y                 50
#define GRAY_WEAK_MAX_BACKGROUND        10.0f
#define GRAY_WEAK_MIN_INNER_CONTRAST    18.0f
#define GRAY_WEAK_MIN_RADIAL_DROP       16.0f
#define GRAY_WEAK_MIN_CONCENTRATION     0.68f
#define GRAY_WEAK_MAX_ELONGATION_SQ     2.80f
#define GRAY_WEAK_MAX_OFFSET_SQ         0.80f
#define GRAY_WEAK_MAX_HALF_AREA         8U
#define GRAY_WEAK_TRACK_GATE_SQ         36.0f
#define GRAY_WEAK_CONFIRM_WINDOW_FRAMES  5U
#define GRAY_WEAK_CONFIRM_HITS           3U
#define GRAY_WEAK_CONFIRM_WINDOW_MASK   31U
#define GRAY_WEAK_CONFIRM_MIN_RESPONSE 3000
#define GRAY_WEAK_RELAXED_MAX_Y        40.0f
#define GRAY_WEAK_LEGACY_FRAMES          3U
#define GRAY_WEAK_LEGACY_HITS            5U
#define GRAY_WEAK_LEGACY_RESPONSE_RANGE 100
#define GRAY_WEAK_PENDING_MAX_MISSES     3U
#define GRAY_WEAK_MAX_MISSES             2U
...
typedef union
{
    struct
    {
        ...
        image_component_span_t span_queue[IMAGE_SPAN_QUEUE_SIZE];
    } components;
    struct
    {
...
static temporal_track_t g_weak_beacon_track;
static unsigned char g_weak_confirm_window;
static unsigned char g_weak_confirm_frame_count;
static unsigned char g_weak_confirm_hit_count;
static signed short g_weak_response_history[GRAY_WEAK_LEGACY_FRAMES];
static unsigned char g_weak_response_count;
static unsigned char g_weak_relaxed_confirmed;
...
static unsigned int g_preprocessed_bright_words[BEACON_IMAGE_H]
                                                    [IMAGE_BINARY_WORD_COUNT];
static unsigned int g_preprocessed_weak_words[GRAY_WEAK_MAX_Y + 1]
                                                  [IMAGE_BINARY_WORD_COUNT];
...
#if IMAGE_USE_CM4_DSP
static const unsigned char (*g_gray_response_image)[BEACON_IMAGE_W];
static unsigned char g_gray_direct_response_enabled;
static unsigned char g_preprocess_expect_direct_response;
#endif
...
#if IMAGE_USE_CM4_DSP
    g_gray_response_image = 0;
    g_gray_direct_response_enabled = 0U;
    g_preprocess_expect_direct_response = 0U;
#endif
}
...
static void beacon_image_reset_temporal(void)
{
    ...
    memset(&g_weak_beacon_track, 0, sizeof(g_weak_beacon_track));
    g_weak_confirm_window = 0U;
    g_weak_confirm_frame_count = 0U;
    g_weak_confirm_hit_count = 0U;
    memset(g_weak_response_history, 0, sizeof(g_weak_response_history));
    g_weak_response_count = 0U;
    g_weak_relaxed_confirmed = 0U;
...
static void preprocess_build_integral_dsp(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W])
{
    int padded_y;
    ...
    for(padded_y = 0; padded_y < GRAY_BEACON_PADDED_H; padded_y++)
    {
        ...
        preprocess_padded_integral_row(
            g_gray_integral[padded_y],
            g_gray_integral[padded_y + 1],
            source);
    }
}

static void preprocess_beacon_words_dsp(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    unsigned char threshold)
{
    unsigned int threshold4 = (unsigned int)threshold * 0x01010101U;
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        ...
        for(word_index = 0; word_index < IMAGE_BINARY_WORD_COUNT;
            word_index++)
        {
            unsigned int bright = g_preprocessed_bright_words[y][word_index];
            unsigned int beacon_bits = 0U;

            while(bright != 0U)
            {
                unsigned int shift =
                    image_count_trailing_zeros(bright) & ~3U;
                unsigned int group_mask = (bright >> shift) & 0x0FU;
                int x = word_index * 32 + (int)shift;
                unsigned int pixels = *(const unsigned int *)&source[x];
                unsigned int selected;
                unsigned int bits;
                ...
                (void)__USUB8(pixels, threshold4);
                selected = __SEL(0x01010101U, 0U);
                bits = ((selected * 0x01020408U) >> 24) & group_mask;
                beacon_bits |= bits << shift;
                bright &= ~(0x0FUL << shift);
            }
            beacon_words[word_index] = beacon_bits;
        }
    }
}
    ...
    unsigned short *bright_pixel_count)
{
    const unsigned int bright_threshold4 =
        (unsigned int)GRAY_BEACON_MIN_PEAK_GRAY * 0x01010101U;
    const unsigned int weak_threshold4 =
        (unsigned int)GRAY_WEAK_MIN_PEAK_GRAY * 0x01010101U;
    unsigned int sum = 0U;
    unsigned int bright = 0U;
    float scene_mean;
    int y;

    if(g_preprocess_expect_direct_response == 0U)
    {
        memset(g_gray_integral[0], 0, sizeof(g_gray_integral[0]));
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        ...
        unsigned int *weak_words =
            (y <= GRAY_WEAK_MAX_Y) ? g_preprocessed_weak_words[y] : 0;
        unsigned char car_threshold = CAR_LAMP_BINARY_THRESHOLD;
        unsigned int car_threshold4;
        unsigned int car_bits = 0U;
        unsigned int bright_bits = 0U;
        unsigned int weak_bits = 0U;
        ...
        for(x = 0; x < BEACON_IMAGE_W; x += 4)
        {
            ...
            if(weak_words != 0)
            {
                (void)__USUB8(pixels, weak_threshold4);
                selected = __SEL(0x01010101U, 0U);
                bits = (selected * 0x01020408U) >> 24;
                weak_bits |= bits << shift;
            }
            ...
            if((shift == 28U) || (x == BEACON_IMAGE_W - 4))
            {
                ...
                if(weak_words != 0)
                {
                    weak_words[word_index] = weak_bits;
                }
                car_bits = 0U;
                bright_bits = 0U;
                weak_bits = 0U;
    ...
    g_gray_direct_response_enabled =
        (bright <= GRAY_BEACON_DIRECT_MAX_BRIGHT_PIXELS) ? 1U : 0U;
    if((g_preprocess_expect_direct_response != 0U) &&
       (g_gray_direct_response_enabled == 0U) &&
       (bright <= GRAY_BEACON_SPARSE_MAX_BRIGHT_PIXELS))
    {
        preprocess_build_integral_dsp(image);
    }
    else if((g_preprocess_expect_direct_response == 0U) &&
            (g_gray_direct_response_enabled == 0U) &&
            (bright <= GRAY_BEACON_SPARSE_MAX_BRIGHT_PIXELS))
    {
        int offset;
    ...
    g_preprocess_expect_direct_response = g_gray_direct_response_enabled;

    if(bright_pixel_count != 0)
    {
        *bright_pixel_count = (unsigned short)bright;
    }
    scene_mean = (float)sum / (float)(BEACON_IMAGE_W * BEACON_IMAGE_H);
    {
        unsigned char beacon_threshold;
        int unused_minimum_area;
        int unused_maximum_area;

        if(gray_component_configuration(
               scene_mean, &beacon_threshold,
               &unused_minimum_area, &unused_maximum_area) != 0U)
        {
            preprocess_beacon_words_dsp(image, beacon_threshold);
        }
    }
    return scene_mean;
}
    ...
    unsigned short *tail)
{
    ...
    g_span_queue[*tail].y = (unsigned char)y;
    g_span_queue[*tail].left = (unsigned char)left;
    g_span_queue[*tail].right = (unsigned char)right;
    (*tail)++;
    return right;
}
    ...
    unsigned char start_y)
{
    ...
    while(head < tail)
    {
        image_component_span_t span = g_span_queue[head++];
        int y = (int)span.y;
        int left = (int)span.left;
        int right = (int)span.right;
...
static signed short gray_integral_response(int x, int y)
{
    const unsigned short *center =
        &g_gray_integral[y + GRAY_BEACON_BOX_PAD]
                        [x + GRAY_BEACON_BOX_PAD];
    unsigned int box3 = (unsigned short)(
        (int)center[2 * GRAY_INTEGRAL_STRIDE + 2] -
        (int)center[-GRAY_INTEGRAL_STRIDE + 2] -
        (int)center[2 * GRAY_INTEGRAL_STRIDE - 1] +
        (int)center[-GRAY_INTEGRAL_STRIDE - 1]);
    unsigned int box9 = (unsigned short)(
        (int)center[5 * GRAY_INTEGRAL_STRIDE + 5] -
        (int)center[-4 * GRAY_INTEGRAL_STRIDE + 5] -
        (int)center[5 * GRAY_INTEGRAL_STRIDE - 4] +
        (int)center[-4 * GRAY_INTEGRAL_STRIDE - 4]);

    return (signed short)(9 * (int)box3 - (int)box9);
}

#if IMAGE_USE_CM4_DSP
static unsigned int gray_integral_load_pair(
...
static unsigned int gray_integral_box_sum_pair(
    const unsigned short *center,
    int bottom_offset,
    int top_offset,
    int right_offset,
    int left_offset)
{
    unsigned int bottom_right =
        gray_integral_load_pair(center + bottom_offset + right_offset);
    unsigned int top_right =
        gray_integral_load_pair(center + top_offset + right_offset);
    unsigned int bottom_left =
        gray_integral_load_pair(center + bottom_offset + left_offset);
    unsigned int top_left =
        gray_integral_load_pair(center + top_offset + left_offset);
    unsigned int sums = __SSUB16(bottom_right, top_right);

    sums = __SSUB16(sums, bottom_left);
    return __SADD16(sums, top_left);
}

static void gray_cache_integral_response_pair(int x, int y)
{
    unsigned int slot = (unsigned int)y & (GRAY_SPARSE_CACHE_ROWS - 1U);
    unsigned int pair_x = (unsigned int)x & ~1U;
    unsigned int word_index = pair_x >> 5;
    unsigned int shift = pair_x & 31U;
    const unsigned short *center =
        &g_gray_integral[y + GRAY_BEACON_BOX_PAD]
                        [pair_x + GRAY_BEACON_BOX_PAD];
    unsigned int box3 = gray_integral_box_sum_pair(
        center,
        2 * GRAY_INTEGRAL_STRIDE,
        -GRAY_INTEGRAL_STRIDE,
        2,
        -1);
    unsigned int box9 = gray_integral_box_sum_pair(
        center,
        5 * GRAY_INTEGRAL_STRIDE,
        -4 * GRAY_INTEGRAL_STRIDE,
        5,
        -4);

    g_gray_sparse_response[slot][pair_x] = (signed short)(
        9 * (int)(box3 & 0xFFFFU) - (int)(box9 & 0xFFFFU));
    g_gray_sparse_response[slot][pair_x + 1U] = (signed short)(
        9 * (int)(box3 >> 16) - (int)(box9 >> 16));
    g_gray_sparse_response_valid[slot][word_index] |= 3UL << shift;
}

static signed short gray_direct_box_response(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    int x,
    int y)
{
    unsigned int box3 = 0U;
    unsigned int box9 = 0U;
    int dx;
    int dy;

    if((x >= GRAY_BEACON_BOX_PAD) &&
       (x < BEACON_IMAGE_W - GRAY_BEACON_BOX_PAD) &&
       (y >= GRAY_BEACON_BOX_PAD) &&
       (y < BEACON_IMAGE_H - GRAY_BEACON_BOX_PAD))
    {
        for(dy = -GRAY_BEACON_BOX_PAD;
            dy <= GRAY_BEACON_BOX_PAD; dy++)
        {
            const unsigned char *row = &image[y + dy][x - 4];

            box9 = __USADA8(*(const unsigned int *)&row[0], 0U, box9);
            box9 = __USADA8(*(const unsigned int *)&row[4], 0U, box9);
            box9 += row[8];
            if((dy >= -1) && (dy <= 1))
            {
                box3 += (unsigned int)row[3] + row[4] + row[5];
            }
        }
    }
    else
    {
        for(dy = -GRAY_BEACON_BOX_PAD;
            dy <= GRAY_BEACON_BOX_PAD; dy++)
        {
            const unsigned char *row = image[gray_reflect_index(
                y + dy, BEACON_IMAGE_H)];

            for(dx = -GRAY_BEACON_BOX_PAD;
                dx <= GRAY_BEACON_BOX_PAD; dx++)
            {
                unsigned int pixel = row[gray_reflect_index(
                    x + dx, BEACON_IMAGE_W)];
...
static signed short gray_cached_integral_response(int x, int y)
{
    ...
    {
#if IMAGE_USE_CM4_DSP
        gray_cache_integral_response_pair(x, y);
#else
...
#if IMAGE_USE_CM4_DSP
static signed short gray_cached_direct_response(int x, int y)
{
    unsigned int slot = (unsigned int)y & (GRAY_SPARSE_CACHE_ROWS - 1U);
    unsigned int word_index = (unsigned int)x >> 5;
    unsigned int bit = 1UL << ((unsigned int)x & 31U);
    ...
    if((g_gray_sparse_response_valid[slot][word_index] & bit) == 0U)
    {
        g_gray_sparse_response[slot][x] =
            gray_direct_box_response(g_gray_response_image, x, y);
        g_gray_sparse_response_valid[slot][word_index] |= bit;
    }
    return g_gray_sparse_response[slot][x];
}
...
static void gray_insert_weak_peak(
    gray_beacon_peak_t peaks[GRAY_WEAK_MAX_PEAKS],
    unsigned char *count,
    signed short response,
    int x,
    int y)
{
    ...
    {
        int dx = x - (int)peaks[index].x;
        int dy = y - (int)peaks[index].y;

        if((dx * dx + dy * dy) <= GRAY_BEACON_PEAK_MERGE_DISTANCE_SQ)
        {
            int move;

            if(response <= peaks[index].response)
            {
                return;
            }
            for(move = index; move + 1 < *count; move++)
            {
                peaks[move] = peaks[move + 1];
            }
            (*count)--;
            break;
        }
    ...
    last = *count;
    if(last >= GRAY_WEAK_MAX_PEAKS)
    {
        last = GRAY_WEAK_MAX_PEAKS - 1;
        if(response <= peaks[last].response)
        {
            return;
        }
...
static unsigned char gray_peak_can_compete(
    unsigned char gray,
    int y,
    signed short response,
    const gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS],
    unsigned char count,
    const gray_beacon_peak_t weak_peaks[GRAY_WEAK_MAX_PEAKS],
    const unsigned char *weak_count)
{
    unsigned char strong_candidate =
        ((gray >= GRAY_BEACON_MIN_PEAK_GRAY) &&
         ((count < GRAY_BEACON_MAX_PEAKS) ||
          (response > peaks[GRAY_BEACON_MAX_PEAKS - 1].response))) ? 1U : 0U;
    unsigned char weak_candidate =
        ((y <= GRAY_WEAK_MAX_Y) &&
         (gray >= GRAY_WEAK_MIN_PEAK_GRAY) &&
         (weak_count != 0) &&
         ((*weak_count < GRAY_WEAK_MAX_PEAKS) ||
          (response > weak_peaks[GRAY_WEAK_MAX_PEAKS - 1].response))) ?
            1U : 0U;

    return (strong_candidate != 0U) || (weak_candidate != 0U);
}
...
#if IMAGE_USE_CM4_DSP
static inline void gray_consider_sparse_peak(
    int x,
    int y,
    const component_t *lamp,
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS],
    unsigned char *count,
    gray_beacon_peak_t weak_peaks[GRAY_WEAK_MAX_PEAKS],
    unsigned char *weak_count)
{
    signed short response;
    ...
    if((response <= 0) ||
       (gray_peak_can_compete(
            g_gray_response_image[y][x], y, response,
            peaks, *count, weak_peaks, weak_count) == 0U))
    {
        return;
    }
    ...
    {
        if(g_gray_response_image[y][x] >= GRAY_BEACON_MIN_PEAK_GRAY)
        {
            gray_insert_peak(peaks, count, response, x, y);
        }
        if((y <= GRAY_WEAK_MAX_Y) && (weak_count != 0))
        {
            gray_insert_weak_peak(
                weak_peaks, weak_count, response, x, y);
        }
    }
}

static inline void gray_consider_direct_peak(
    int x,
    int y,
    const component_t *lamp,
    const component_t *temporal_lamp,
    ...
    unsigned char *weak_count)
{
    ...
    response = gray_cached_direct_response(x, y);
    if((response <= 0) ||
       (gray_peak_can_compete(
            g_gray_response_image[y][x], y, response,
            peaks, *count, weak_peaks, weak_count) == 0U))
    ...
    if((response < gray_cached_direct_response(x - 1, y - 1)) ||
       (response < gray_cached_direct_response(x - 1, y)) ||
       (response < gray_cached_direct_response(x - 1, y + 1)) ||
       (response < gray_cached_direct_response(x, y - 1)) ||
       (response < gray_cached_direct_response(x, y + 1)) ||
       (response < gray_cached_direct_response(x + 1, y - 1)) ||
       (response < gray_cached_direct_response(x + 1, y)) ||
       (response < gray_cached_direct_response(x + 1, y + 1)))
...
#if IMAGE_USE_CM4_DSP
static unsigned char gray_find_direct_peaks(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    const component_t *lamp,
    const component_t *temporal_lamp,
    float lamp_cos,
    ...
    unsigned char *weak_count)
{
    ...
    for(y = 2; y <= BEACON_IMAGE_H - 3; y++)
    {
        ...
        {
            unsigned int bits =
                g_preprocessed_bright_words[y][word_index];

            if((y <= GRAY_WEAK_MAX_Y) && (weak_count != 0))
            {
                bits |= g_preprocessed_weak_words[y][word_index];
            }
            ...
            while(bits != 0U)
            {
                ...
                int peak_x = word_index * 32 + (int)bit;

                bits &= bits - 1U;
                gray_consider_direct_peak(
                    peak_x, y, lamp, temporal_lamp,
                    lamp_cos, lamp_sin,
                    temporal_lamp_cos, temporal_lamp_sin,
                    peaks, &count, weak_peaks, weak_count);
            }
        }
    }
    return count;
    ...
    unsigned char *weak_count)
{
...
#if IMAGE_USE_CM4_DSP
    g_gray_response_image = image;
    if(g_gray_direct_response_enabled != 0U)
    {
        return gray_find_direct_peaks(
            image, lamp, temporal_lamp,
            lamp_cos, lamp_sin,
            temporal_lamp_cos, temporal_lamp_sin,
            peaks, weak_peaks, weak_count);
    }
#endif
    ...
    for(y = 2; y <= BEACON_IMAGE_H - 3; y++)
    {
        ...
        for(x = 0; x < IMAGE_BINARY_WORD_COUNT; x++)
        {
            ...
            if((y <= GRAY_WEAK_MAX_Y) && (weak_count != 0))
            {
                bits |= g_preprocessed_weak_words[y][x];
            }
        ...
        for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
        {
            ...
            if((image[y][x] < ((weak_count != 0) ?
                                GRAY_WEAK_MIN_PEAK_GRAY :
                                GRAY_BEACON_MIN_PEAK_GRAY)) ||
    ...
    unsigned char *weak_count)
{
    ...
    memset(weak_peaks, 0,
           sizeof(gray_beacon_peak_t) * GRAY_WEAK_MAX_PEAKS);
    if(weak_count != 0) *weak_count = 0U;
    ...
    for(y = 1; y <= BEACON_IMAGE_H - 2; y++)
    {
        ...
        if((y >= 3) && (y <= BEACON_IMAGE_H - 2))
        {
            ...
            for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
            {
                ...
                if((response <= 0) ||
                   (image[center_y][x] < ((weak_count != 0) ?
                                           GRAY_WEAK_MIN_PEAK_GRAY :
                                           GRAY_BEACON_MIN_PEAK_GRAY)) ||
                   (gray_peak_can_compete(
                        image[center_y][x], center_y, response,
                        peaks, count, weak_peaks, weak_count) == 0U) ||
                ...
                {
                    if(image[center_y][x] >= GRAY_BEACON_MIN_PEAK_GRAY)
                    {
                        gray_insert_peak(peaks, &count, response, x, center_y);
                    }
                    if((center_y <= GRAY_WEAK_MAX_Y) && (weak_count != 0))
                    {
                        gray_insert_weak_peak(
                            weak_peaks, weak_count,
                            response, x, center_y);
                    }
                }
            }
...
static unsigned char gray_point_features_valid(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    int x,
    int y,
    float scene_mean,
    ...
    float *refined_y)
{
    ...
    return gray_point_features_valid(
        image, x, y, scene_mean, &features,
        area, refined_x, refined_y);
}

static unsigned char gray_weak_features_valid(
    const gray_beacon_features_t *features,
    int x,
    int y,
    float *area,
    ...
    float *refined_y)
{
    if((features == 0) || (y > GRAY_WEAK_MAX_Y))
    {
        return 0U;
    }
    if((features->background > GRAY_WEAK_MAX_BACKGROUND) ||
       (features->half_area == 0U) ||
       (features->half_area > GRAY_WEAK_MAX_HALF_AREA) ||
       (features->inner_contrast < GRAY_WEAK_MIN_INNER_CONTRAST) ||
       (features->radial_drop < GRAY_WEAK_MIN_RADIAL_DROP) ||
       (features->concentration < GRAY_WEAK_MIN_CONCENTRATION) ||
       (features->elongation_sq > GRAY_WEAK_MAX_ELONGATION_SQ) ||
       (features->offset_sq > GRAY_WEAK_MAX_OFFSET_SQ))
    {
        return 0U;
    }
    if(area != 0) *area = (float)features->half_area;
    if(refined_x != 0) *refined_x = (float)x + features->centroid_x;
    if(refined_y != 0) *refined_y = (float)y + features->centroid_y;
    return 1U;
}
...
static void find_beacons(
    ...
    unsigned short bright_pixel_count,
    beacon_result_t *result,
    beacon_circle_t *weak_measurement,
    signed short *weak_response)
{
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS];
    gray_beacon_peak_t weak_peaks[GRAY_WEAK_MAX_PEAKS];
    gray_beacon_candidate_t candidates[GRAY_BEACON_MAX_CANDIDATES];
    unsigned char peak_count;
    unsigned char weak_peak_count = 0U;
    unsigned char candidate_count = 0U;
    unsigned char index;
    float weak_best_distance = GRAY_WEAK_TRACK_GATE_SQ;
    float weak_best_y = (float)(GRAY_WEAK_MAX_Y + 1);

    memset(candidates, 0, sizeof(candidates));
    if(weak_measurement != 0)
    {
        memset(weak_measurement, 0, sizeof(*weak_measurement));
    }
    if(weak_response != 0) *weak_response = 0;
    peak_count = gray_find_box_peaks(
        image, lamp, temporal_lamp, bright_pixel_count,
        peaks, weak_peaks,
        (scene_mean < GRAY_BEACON_DARK_SCENE_MEAN) ?
            &weak_peak_count : 0);
    for(index = 0U; index < peak_count; index++)
    {
        ...
        if(gray_local_features(image, x, y, &features, 0U) == 0U)
        {
            continue;
        }
        strong_candidate = gray_point_features_valid(
            image, x, y, scene_mean, &features,
            &area, &refined_x, &refined_y);
        if(strong_candidate != 0U)
        {
            gray_add_candidate(
    ...
    if(scene_mean < GRAY_BEACON_DARK_SCENE_MEAN)
    {
        for(index = 0U; index < weak_peak_count; index++)
        {
            int x = weak_peaks[index].x;
            int y = weak_peaks[index].y;
            float area;
            float refined_x = (float)x;
            float refined_y = (float)y;
            gray_beacon_features_t features;
            ...
            if((gray_local_features(image, x, y, &features, 0U) != 0U) &&
               (weak_measurement != 0) &&
               (gray_weak_features_valid(
                    &features, x, y,
                    &area, &refined_x, &refined_y) != 0U))
            {
                float centered_x =
                    refined_x - (float)BEACON_IMAGE_W * 0.5f;
                float centered_y =
                    refined_y - (float)BEACON_IMAGE_H * 0.5f;
                float distance = GRAY_WEAK_TRACK_GATE_SQ;
                unsigned char select = 0U;

                if(g_weak_beacon_track.active != 0U)
                {
                    float dx = centered_x -
                               (g_weak_beacon_track.x +
                                g_weak_beacon_track.vx);
                    float dy = centered_y -
                               (g_weak_beacon_track.y +
                                g_weak_beacon_track.vy);

                    distance = dx * dx + dy * dy;
                    if(distance <= weak_best_distance)
                    {
                        select = 1U;
                    }
                }
                else if(refined_y < weak_best_y)
                {
                    select = 1U;
                }
                if(select != 0U)
                {
                    weak_measurement->x = centered_x;
                    weak_measurement->y = centered_y;
                    weak_measurement->area = area;
                    weak_measurement->radius = sqrtf(area / PI_F);
                    weak_measurement->valid = 1U;
                    if(weak_response != 0)
                    {
                        *weak_response = weak_peaks[index].response;
                    }
                    weak_best_distance = distance;
                    weak_best_y = refined_y;
                }
            }
        }
    }
...
static void reset_weak_beacon_track(void)
{
    reset_track(&g_weak_beacon_track);
    g_weak_confirm_window = 0U;
    g_weak_confirm_frame_count = 0U;
    g_weak_confirm_hit_count = 0U;
    memset(g_weak_response_history, 0, sizeof(g_weak_response_history));
...
static void record_weak_beacon_confirmation(unsigned char hit)
{
    if(g_weak_confirm_frame_count >= GRAY_WEAK_CONFIRM_WINDOW_FRAMES)
    {
        if((g_weak_confirm_window &
            (1U << (GRAY_WEAK_CONFIRM_WINDOW_FRAMES - 1U))) != 0U)
        {
            g_weak_confirm_hit_count--;
        }
    }
    else
    {
        g_weak_confirm_frame_count++;
    }
    g_weak_confirm_window = (unsigned char)(
        ((unsigned int)g_weak_confirm_window << 1U) &
        GRAY_WEAK_CONFIRM_WINDOW_MASK);
    if(hit != 0U)
    {
        g_weak_confirm_window |= 1U;
        g_weak_confirm_hit_count++;
    }
}

static void start_weak_beacon_track(
    const beacon_circle_t *measurement,
    signed short response)
{
    start_pending_beacon(&g_weak_beacon_track, measurement);
    g_weak_confirm_window = 0U;
    g_weak_confirm_frame_count = 0U;
    g_weak_confirm_hit_count = 0U;
    record_weak_beacon_confirmation(
        (response >= GRAY_WEAK_CONFIRM_MIN_RESPONSE) ? 1U : 0U);
    memset(g_weak_response_history, 0, sizeof(g_weak_response_history));
    g_weak_response_history[0] = response;
    g_weak_response_count = 1U;
    g_weak_relaxed_confirmed = 0U;
}

static unsigned char weak_beacon_legacy_response_valid(void)
{
    signed short minimum;
    signed short maximum;
    unsigned char index;

    if((g_weak_beacon_track.hits < GRAY_WEAK_LEGACY_HITS) ||
       (g_weak_response_count < GRAY_WEAK_LEGACY_FRAMES))
    {
        return 0U;
    }
    minimum = g_weak_response_history[0];
    maximum = minimum;
    for(index = 1U; index < GRAY_WEAK_LEGACY_FRAMES; index++)
    {
        if(g_weak_response_history[index] < minimum)
        {
            minimum = g_weak_response_history[index];
        }
        if(g_weak_response_history[index] > maximum)
        {
            maximum = g_weak_response_history[index];
        }
    }
    return ((maximum - minimum) <= GRAY_WEAK_LEGACY_RESPONSE_RANGE) ? 1U : 0U;
}

static void update_weak_beacon_track(
    const beacon_circle_t *measurement,
    signed short response)
{
    ...
    {
        if((g_weak_beacon_track.active != 0U) &&
           (g_weak_beacon_track.confirmed == 0U) &&
           (g_weak_beacon_track.misses < GRAY_WEAK_PENDING_MAX_MISSES))
        {
            record_weak_beacon_confirmation(0U);
            g_weak_beacon_track.x += g_weak_beacon_track.vx;
            g_weak_beacon_track.y += g_weak_beacon_track.vy;
            g_weak_beacon_track.misses++;
            return;
        }
        if((g_weak_beacon_track.confirmed != 0U) &&
           (g_weak_beacon_track.misses < GRAY_WEAK_MAX_MISSES))
        {
            g_weak_beacon_track.x += g_weak_beacon_track.vx;
            g_weak_beacon_track.y += g_weak_beacon_track.vy;
            g_weak_beacon_track.misses++;
        ...
        reset_weak_beacon_track();
        return;
    }
    if(g_weak_beacon_track.active == 0U)
    {
        start_weak_beacon_track(measurement, response);
        return;
    }
    if(square_distance(
           g_weak_beacon_track.x + g_weak_beacon_track.vx,
           g_weak_beacon_track.y + g_weak_beacon_track.vy,
           measurement->x, measurement->y) > GRAY_WEAK_TRACK_GATE_SQ)
    {
        start_weak_beacon_track(measurement, response);
        return;
    }
    update_beacon_track(&g_weak_beacon_track, measurement);
    g_weak_beacon_track.hits++;
    record_weak_beacon_confirmation(
        (response >= GRAY_WEAK_CONFIRM_MIN_RESPONSE) ? 1U : 0U);
    if(g_weak_response_count < GRAY_WEAK_LEGACY_FRAMES)
    {
        g_weak_response_history[g_weak_response_count++] = response;
    }
    else
    {
        g_weak_response_history[0] = g_weak_response_history[1];
        g_weak_response_history[1] = g_weak_response_history[2];
        g_weak_response_history[2] = response;
    }
    if((g_weak_beacon_track.confirmed == 0U) &&
       (weak_beacon_legacy_response_valid() != 0U))
    {
        g_weak_beacon_track.confirmed = 1U;
    }
    else if((g_weak_beacon_track.confirmed == 0U) &&
            (g_weak_confirm_frame_count >= GRAY_WEAK_CONFIRM_WINDOW_FRAMES) &&
            (g_weak_confirm_hit_count >= GRAY_WEAK_CONFIRM_HITS) &&
            (g_weak_beacon_track.y + (float)BEACON_IMAGE_H * 0.5f <=
             GRAY_WEAK_RELAXED_MAX_Y))
    {
        g_weak_beacon_track.confirmed = 1U;
        g_weak_relaxed_confirmed = 1U;
    }
}

static void append_confirmed_weak_beacon(beacon_result_t *result)
{
    int index;
    int count;

    if((result == 0) || (g_weak_beacon_track.confirmed == 0U) ||
       ((g_weak_relaxed_confirmed != 0U) &&
        (g_weak_beacon_track.y + (float)BEACON_IMAGE_H * 0.5f >
         GRAY_WEAK_RELAXED_MAX_Y)))
    {
    ...
    for(index = 0; index < count; index++)
    {
        if(square_distance(
               ...
               g_weak_beacon_track.x, g_weak_beacon_track.y) <=
           GRAY_BEACON_PEAK_MERGE_DISTANCE_SQ)
        {
            return;
        }
    }
    ...
    result->beacons[count].x = g_weak_beacon_track.x;
    result->beacons[count].y = g_weak_beacon_track.y;
    result->beacons[count].radius = g_weak_beacon_track.radius;
    result->beacons[count].area = g_weak_beacon_track.area;
    result->beacons[count].valid = 1U;
    result->beacon_count = (unsigned char)(count + 1);
}
...
static unsigned char beacon_output_is_lower_small(
    const beacon_circle_t *beacon)
{
    ...
    return (((beacon->y + (float)BEACON_IMAGE_H * 0.5f) >=
             (float)BEACON_LOWER_SMALL_Y) &&
            (beacon_area(beacon) < (float)BEACON_MIN_COMPONENT_AREA)) ?
               1U : 0U;
}
...
static void beacon_output_predict(beacon_result_t *result)
{
    ...
    if(((g_b0_track.y + (float)BEACON_IMAGE_H * 0.5f) >=
        (float)BEACON_LOWER_SMALL_Y) &&
       (g_b0_track.area < (float)BEACON_MIN_COMPONENT_AREA))
    {
        reset_track(&g_b0_track);
        return;
    }
    ...
    beacon_result_t *result)
{
    ...
    for(source_index = 0U; source_index < source_count; source_index++)
    {
        ...
        if((source->valid == 0U) ||
           (beacon_output_is_lower_small(source) != 0U) ||
    ...
    beacon_result_t *result)
{
    ...
    find_beacons(image, &lamp, &temporal_lamp,
                 ...
                 &weak_measurement, &weak_response);
    update_weak_beacon_track(&weak_measurement, weak_response);
    update_temporal_car(result, (has_lamp != 0U) ? &lamp : 0);
    beacon_output_finalize(image, result);
    append_confirmed_weak_beacon(result);
}
...
volatile uint32 g_image_result_sequence;                                          /* 已完成算法处理并发布的结果序号。 */

static uint32 s_image_latched_frame_sequence;                                     // 最近锁存帧的来源序号
...
static uint8 image_latch_frame(void)
{
    ...
    /* 复制期间若中断发布了新帧，重新锁存，保证算法不会消费撕裂图像。 */
    do
    {
    ...
    s_image_latched_frame_sequence = frame_sequence;
    return 1U;
}
...
void image_init(void)
{
    ...
    g_image_result_sequence = 0U;
    s_image_latched_frame_sequence = 0U;
