...
uint16 g_mt9v03x_exp_time = 500U;

static desktop_telemetry_t g_desktop_telemetry;
static uint32_t g_desktop_processed_frame_count;
...
void beacon_image_init(void)
{
    ...
    g_desktop_processed_frame_count = 0U;
    image_horizon_init();
    image_init();
}
    ...
    desktop_result_t *result)
{
    ...
    image_update();
    g_desktop_processed_frame_count++;

    for(index = 0U;
        (index < IMAGE_MAX_BEACON_COUNT) &&
        (result->beacon_count < DESKTOP_MAX_BEACONS);
        index++)
    {
        ...
        target->valid = 1U;
        result->circles[result->beacon_count] = *target;
        result->beacon_count++;
    }
    result->count = result->beacon_count;

    for(index = 0U;
        (index < IMAGE_MAX_CAR_LAMP_COUNT) &&
        (result->car_lamp_count < DESKTOP_MAX_CAR_LAMPS);
        index++)
    {
...
static void copy_stream_binary(
    int binary_mode,
    unsigned char binary[DESKTOP_IMAGE_H][DESKTOP_IMAGE_W])
{
    unsigned char saved_frame[DESKTOP_IMAGE_H][DESKTOP_IMAGE_W];
    const int saved_mode = stream_mode;
    if (binary == NULL)
    {
        return;
    }
    memcpy(saved_frame, image_get_frame_buffer(), sizeof(saved_frame));
    stream_mode = binary_mode;
    memcpy(binary, image_prepare_stream_frame(),
           DESKTOP_IMAGE_W * DESKTOP_IMAGE_H);
    memcpy(image_get_frame_buffer(), saved_frame, sizeof(saved_frame));
...
void beacon_image_debug_binary(
    ...
    unsigned char binary[DESKTOP_IMAGE_H][DESKTOP_IMAGE_W])
{
    const int binary_mode = (stream_mode == IMAGE_STREAM_MODE_LAMP_BINARY) ?
        IMAGE_STREAM_MODE_LAMP_BINARY : IMAGE_STREAM_MODE_BEACON_BINARY;
    (void)image;
    copy_stream_binary(binary_mode, binary);
}
...
{
    (void)image;
    copy_stream_binary(IMAGE_STREAM_MODE_LAMP_BINARY, binary);
}

unsigned char beacon_image_debug_car_lamp_pixel_areas(
    unsigned short areas[DESKTOP_MAX_CAR_LAMPS])
{
    uint8 index;
    uint8 count = 0U;
    if (areas == NULL)
    ...
    for (index = 0U; index < IMAGE_MAX_CAR_LAMP_COUNT &&
                       count < DESKTOP_MAX_CAR_LAMPS; index++)
    {
        const car_lamp_data *lamp = &g_image_data.car_lamp_data[index];
...
{
    return g_desktop_processed_frame_count;
}
