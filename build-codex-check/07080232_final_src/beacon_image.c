...
#define BEACON_WEAK_FOOTPRINT_MAX 16
...
#define B0_INIT_CONFIRM_FRAMES    3
#define CAR_LAMP_INIT_CONFIRM_FRAMES 2
...
static void update_temporal_beacon(beacon_result_t *result)
{
    ...
    if (selected < 0)
    {
        ...
        g_b0_track.hits = B0_INIT_CONFIRM_FRAMES;
        output_temporal_beacon(&g_b0_track, result);
        return;
    }
...
static void update_temporal_car(beacon_result_t *result)
{
    ...
    g_car_track.hits++;
    if (g_car_track.confirmed == 0 &&
        g_car_track.hits >= CAR_LAMP_INIT_CONFIRM_FRAMES)
    {
