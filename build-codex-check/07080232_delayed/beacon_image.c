...
#define BEACON_CENTER_MAX_MISSES  4
#define BEACON_EDGE_MAX_MISSES    16
#define BEACON_TEMPORAL_SIDE_X    65.0f
#define BEACON_TEMPORAL_TOP_Y    -14.0f
#define BEACON_TEMPORAL_BOTTOM_Y  34.0f
...
static unsigned char g_beacon_edge_flags[BEACON_MAX_BEACON_COUNT];
    ...
    beacon_result_t *result)
{
    ...
    for (i = slot - 1; i >= 0; i--)
    {
        ...
        g_beacon_edge_flags[i + 1] = g_beacon_edge_flags[i];
    }
    ...
    g_beacon_edge_flags[i + 1] = is_edge;
}
    ...
    beacon_result_t *result)
{
    ...
    memset(g_beacon_edge_flags, 0, sizeof(g_beacon_edge_flags));
...
static unsigned char temporal_beacon_max_misses(const temporal_track_t *track)
{
    float x = track->x + track->vx;
    float y = track->y + track->vy;
    return (x <= -BEACON_TEMPORAL_SIDE_X ||
            x >= BEACON_TEMPORAL_SIDE_X ||
            y <= BEACON_TEMPORAL_TOP_Y ||
            y >= BEACON_TEMPORAL_BOTTOM_Y) ?
               BEACON_EDGE_MAX_MISSES : BEACON_CENTER_MAX_MISSES;
}
...
static void update_temporal_beacon(beacon_result_t *result)
{
    ...
    const float gate = KALMAN_GATE_DISTANCE;
    ...
    if (count <= 0)
    {
        if (g_b0_track.confirmed != 0 &&
            g_b0_track.misses < temporal_beacon_max_misses(&g_b0_track))
        {
    ...
    else
    {
        ...
        {
            if (g_beacon_edge_flags[selected] == 0)
            {
                break;
            }
        }
    ...
    if (g_b0_track.confirmed == 0)
    {
        ...
        if (g_b0_track.confirmed != 0 || g_beacon_edge_flags[selected] == 0)
        {
            output_temporal_beacon(&g_b0_track, result);
        }
        return;
