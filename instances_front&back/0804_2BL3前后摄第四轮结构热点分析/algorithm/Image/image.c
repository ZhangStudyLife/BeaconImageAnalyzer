...
enum
{
    IMAGE_PROFILE_FRAMES = 0,
    IMAGE_PROFILE_SCENE_MEAN,
    IMAGE_PROFILE_CAR_THRESHOLD,
    IMAGE_PROFILE_CAR_PRIMARY,
    IMAGE_PROFILE_CAR_GRAY,
    IMAGE_PROFILE_CAR_TEMPORAL,
    IMAGE_PROFILE_BEACON_TOTAL,
    IMAGE_PROFILE_FINALIZE,
    IMAGE_PROFILE_PEAK_TOTAL,
    IMAGE_PROFILE_SPARSE_PATH_COUNT,
    IMAGE_PROFILE_DENSE_PATH_COUNT,
    IMAGE_PROFILE_PEAK_FEATURES,
    IMAGE_PROFILE_COMPONENT_CANDIDATES,
    IMAGE_PROFILE_CANDIDATE_STORE,
    IMAGE_PROFILE_PEAK_COUNT_TOTAL,
    IMAGE_PROFILE_CANDIDATE_COUNT_TOTAL,
    IMAGE_PROFILE_BRIGHT_PIXEL_TOTAL,
    IMAGE_PROFILE_COMPONENT_THRESHOLD,
    IMAGE_PROFILE_COMPONENT_SCAN,
    IMAGE_PROFILE_SPARSE_PATH_TIME,
    IMAGE_PROFILE_DENSE_PATH_TIME,
    IMAGE_PROFILE_CAR_PRIMARY_HITS,
    IMAGE_PROFILE_CAR_GRAY_FALLBACKS,
    IMAGE_PROFILE_CAR_TEMPORAL_FALLBACKS,
    IMAGE_PROFILE_COMPONENT_ACTIVE_FRAMES,
    IMAGE_PROFILE_PEAK_CANDIDATE_COUNT_TOTAL,
    IMAGE_PROFILE_COMPONENT_ADDED_FRAMES,
    IMAGE_PROFILE_COMPONENT_ADDED_TOTAL,
    IMAGE_PROFILE_COMPONENT_CHANGED_OUTPUT_FRAMES,
    IMAGE_PROFILE_COMPONENT_CHANGED_FROM_ZERO,
    IMAGE_PROFILE_COMPONENT_CHANGED_FROM_ONE,
    IMAGE_PROFILE_COMPONENT_CHANGED_FROM_TWO_PLUS,
    IMAGE_PROFILE_COMPONENT_GROW_TIME,
    IMAGE_PROFILE_COMPONENT_VALIDATE_TIME,
    IMAGE_PROFILE_COMPONENT_GROW_CALLS,
    IMAGE_PROFILE_COMPONENT_SIZE_VALID_CALLS,
    IMAGE_PROFILE_COMPONENT_POINT_SOURCE_CALLS,
    IMAGE_PROFILE_COMPONENT_ACCEPTED_CALLS,
    IMAGE_PROFILE_PEAK_ONLY_OUTPUT_ZERO,
    IMAGE_PROFILE_PEAK_ONLY_OUTPUT_ONE,
    IMAGE_PROFILE_PEAK_ONLY_OUTPUT_TWO_PLUS,
    IMAGE_PROFILE_COUNT
};

static uint64_t g_image_host_profile[IMAGE_PROFILE_COUNT];

void image_host_profile_reset(void)
{
    memset(g_image_host_profile, 0, sizeof(g_image_host_profile));
}

void image_host_profile_read(uint64_t output[IMAGE_PROFILE_COUNT])
{
    if(output != 0)
    {
        memcpy(output, g_image_host_profile, sizeof(g_image_host_profile));
    }
}
#endif
    ...
    unsigned char threshold)
{
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        threshold_row_dsp(image[y], g_binary[y],
                          BEACON_IMAGE_W, threshold);
    }
#else
    int index;
    const unsigned char *source = &image[0][0];
    unsigned char *destination = &g_binary[0][0];

    for(index = 0; index < BEACON_IMAGE_W * BEACON_IMAGE_H; index++)
    {
        destination[index] = (source[index] >= threshold) ? 255U : 0U;
    }
#endif
}
    ...
    int shape_max_area)
{
    ...
    tail++;
    g_binary[start_y][start_x] = IMAGE_BINARY_VISITED;
    ...
    while(head < tail)
    {
        ...
        for(i = 0; i < 8; i++)
        {
            ...
            if((g_binary[ny][nx] != IMAGE_BINARY_FOREGROUND) ||
               (tail >= IMAGE_QUEUE_SIZE))
            {
                continue;
            }

            g_binary[ny][nx] = IMAGE_BINARY_VISITED;
    ...
    component_t *best_lamp)
{
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
            ...
            if(g_binary[y][x] != IMAGE_BINARY_FOREGROUND)
            {
                continue;
            }
...
#if defined(IMAGE_HOST_PROFILE)
        g_image_host_profile[IMAGE_PROFILE_SPARSE_PATH_COUNT]++;
        path_start = __rdtsc();
#endif
        sparse_count = gray_find_sparse_integral_peaks(
            image, lamp, temporal_lamp,
            lamp_cos, lamp_sin,
            temporal_lamp_cos, temporal_lamp_sin,
            peaks);
#if defined(IMAGE_HOST_PROFILE)
        g_image_host_profile[IMAGE_PROFILE_SPARSE_PATH_TIME] +=
            __rdtsc() - path_start;
#endif
        return sparse_count;
    }
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_DENSE_PATH_COUNT]++;
    path_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_DENSE_PATH_TIME] +=
        __rdtsc() - path_start;
#endif
    return count;
}
    ...
    unsigned char *count)
{
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
...
#if defined(IMAGE_HOST_PROFILE)
            component_start = __rdtsc();
            g_image_host_profile[IMAGE_PROFILE_COMPONENT_GROW_CALLS]++;
#endif
            if(evaluated >= BEACON_MAX_EVALUATED_COMPONENTS)
            {
                (void)grow_component((unsigned char)x, (unsigned char)y, 0, -1);
#if defined(IMAGE_HOST_PROFILE)
                g_image_host_profile[IMAGE_PROFILE_COMPONENT_GROW_TIME] +=
                    __rdtsc() - component_start;
#endif
                continue;
            }
            comp = grow_component(
...
#if defined(IMAGE_HOST_PROFILE)
            g_image_host_profile[IMAGE_PROFILE_COMPONENT_SIZE_VALID_CALLS]++;
            component_start = __rdtsc();
#endif
            center_x = (int)(comp.cx + 0.5f);
            center_y = (int)(comp.cy + 0.5f);
            ...
            {
#if defined(IMAGE_HOST_PROFILE)
                g_image_host_profile[IMAGE_PROFILE_COMPONENT_POINT_SOURCE_CALLS]++;
#endif
            ...
            {
#if defined(IMAGE_HOST_PROFILE)
                g_image_host_profile[IMAGE_PROFILE_COMPONENT_ACCEPTED_CALLS]++;
#endif
                gray_add_candidate(candidates, count, center_x, center_y,
                                   (float)comp.area);
            }
#if defined(IMAGE_HOST_PROFILE)
            g_image_host_profile[IMAGE_PROFILE_COMPONENT_VALIDATE_TIME] +=
                __rdtsc() - component_start;
#endif
        }
    }
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_COMPONENT_SCAN] +=
        __rdtsc() - profile_start;
#endif
}
...
#if defined(IMAGE_HOST_PROFILE)
    uint64_t profile_start;
    beacon_result_t peak_only_result;
    unsigned char peak_candidate_count;
#endif
...
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_PEAK_TOTAL] +=
        __rdtsc() - profile_start;
    g_image_host_profile[IMAGE_PROFILE_PEAK_COUNT_TOTAL] += peak_count;
    profile_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_PEAK_FEATURES] +=
        __rdtsc() - profile_start;
    peak_candidate_count = candidate_count;
    g_image_host_profile[IMAGE_PROFILE_PEAK_CANDIDATE_COUNT_TOTAL] +=
        peak_candidate_count;
    memset(&peak_only_result, 0, sizeof(peak_only_result));
    gray_store_candidates(image, candidates, peak_candidate_count,
                          scene_mean, &peak_only_result);
    if(peak_only_result.beacon_count == 0U)
    {
        g_image_host_profile[IMAGE_PROFILE_PEAK_ONLY_OUTPUT_ZERO]++;
    }
    else if(peak_only_result.beacon_count == 1U)
    {
        g_image_host_profile[IMAGE_PROFILE_PEAK_ONLY_OUTPUT_ONE]++;
    }
    else
    {
        g_image_host_profile[IMAGE_PROFILE_PEAK_ONLY_OUTPUT_TWO_PLUS]++;
    }
    if((scene_mean < GRAY_BEACON_DARK_SCENE_MEAN) ||
       (scene_mean >= GRAY_BEACON_BRIGHT_SCENE_MEAN))
    {
        g_image_host_profile[IMAGE_PROFILE_COMPONENT_ACTIVE_FRAMES]++;
    }
    profile_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_COMPONENT_CANDIDATES] +=
        __rdtsc() - profile_start;
    profile_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_CANDIDATE_STORE] +=
        __rdtsc() - profile_start;
    g_image_host_profile[IMAGE_PROFILE_CANDIDATE_COUNT_TOTAL] +=
        candidate_count;
    if(candidate_count > peak_candidate_count)
    {
        g_image_host_profile[IMAGE_PROFILE_COMPONENT_ADDED_FRAMES]++;
        g_image_host_profile[IMAGE_PROFILE_COMPONENT_ADDED_TOTAL] +=
            candidate_count - peak_candidate_count;
    }
    if((peak_only_result.beacon_count != result->beacon_count) ||
       (memcmp(peak_only_result.beacons, result->beacons,
               sizeof(result->beacons)) != 0))
    {
        g_image_host_profile[IMAGE_PROFILE_COMPONENT_CHANGED_OUTPUT_FRAMES]++;
        if(peak_candidate_count == 0U)
        {
            g_image_host_profile[IMAGE_PROFILE_COMPONENT_CHANGED_FROM_ZERO]++;
        }
        else if(peak_candidate_count == 1U)
        {
            g_image_host_profile[IMAGE_PROFILE_COMPONENT_CHANGED_FROM_ONE]++;
        }
        else
        {
            g_image_host_profile[IMAGE_PROFILE_COMPONENT_CHANGED_FROM_TWO_PLUS]++;
        }
    }
#endif
}
    ...
    beacon_result_t *result)
{
...
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_SCENE_MEAN] +=
        __rdtsc() - profile_start;
    g_image_host_profile[IMAGE_PROFILE_BRIGHT_PIXEL_TOTAL] +=
        bright_pixel_count;
    profile_start = __rdtsc();
#endif
    threshold_car_lamp_image(image);
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_CAR_THRESHOLD] +=
        __rdtsc() - profile_start;
    profile_start = __rdtsc();
#endif
    has_lamp = find_car_lamp(image, &lamp);
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_CAR_PRIMARY] +=
        __rdtsc() - profile_start;
#endif
    if(has_lamp == 0U)
    {
#if defined(IMAGE_HOST_PROFILE)
        g_image_host_profile[IMAGE_PROFILE_CAR_GRAY_FALLBACKS]++;
        profile_start = __rdtsc();
#endif
        has_lamp = find_gray_car_lamp(image, scene_mean, &lamp);
#if defined(IMAGE_HOST_PROFILE)
        g_image_host_profile[IMAGE_PROFILE_CAR_GRAY] +=
            __rdtsc() - profile_start;
#endif
    }
#if defined(IMAGE_HOST_PROFILE)
    ...
    {
        g_image_host_profile[IMAGE_PROFILE_CAR_PRIMARY_HITS]++;
    }
#endif
    if(has_lamp == 0)
    {
#if defined(IMAGE_HOST_PROFILE)
        g_image_host_profile[IMAGE_PROFILE_CAR_TEMPORAL_FALLBACKS]++;
        profile_start = __rdtsc();
#endif
        has_lamp = find_temporal_car_lamp(image, &lamp);
#if defined(IMAGE_HOST_PROFILE)
        g_image_host_profile[IMAGE_PROFILE_CAR_TEMPORAL] +=
            __rdtsc() - profile_start;
#endif
    }
    if(has_lamp == 0)
...
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_BEACON_TOTAL] +=
        __rdtsc() - profile_start;
    profile_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PROFILE)
    g_image_host_profile[IMAGE_PROFILE_FINALIZE] +=
        __rdtsc() - profile_start;
    g_image_host_profile[IMAGE_PROFILE_FRAMES]++;
#endif
}
