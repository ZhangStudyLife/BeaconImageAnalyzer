    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        ...
        if((y >= 3) && (y <= BEACON_IMAGE_H - 2))
        {
            ...
            for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
            {
                ...
                if(dx > 1)
                {
                    if((gray_point_in_lamp(x, center_y, lamp) == 0U) &&
                       (gray_point_in_lamp(x, center_y, temporal_lamp) == 0U) &&
    ...
    int center_y)
{
        ...
        dy <= GRAY_BEACON_PATCH_RADIUS; dy++)
    {
            ...
            dx <= GRAY_BEACON_PATCH_RADIUS; dx++)
        {
            ...
            {
                histogram[gray_pixel(image, center_x + dx, center_y + dy)]++;
                count++;
            }
        }
    }
    ...
    return ((float)low + (float)high) * 0.5f;
}
    ...
    gray_beacon_features_t *features)
{
    ...
    background = gray_ring_median(image, center_x, center_y);
