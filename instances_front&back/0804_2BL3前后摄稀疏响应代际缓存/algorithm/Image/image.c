...
static unsigned int g_gray_sparse_response_cache[BEACON_IMAGE_H][BEACON_IMAGE_W];
static unsigned short g_gray_sparse_response_generation;
...
{
    unsigned int entry = g_gray_sparse_response_cache[y][x];

    if((unsigned short)(entry >> 16) != g_gray_sparse_response_generation)
    {
        signed short response = gray_integral_response(x, y);

        entry = ((unsigned int)g_gray_sparse_response_generation << 16) |
                (unsigned short)((int)response + 32768);
        g_gray_sparse_response_cache[y][x] = entry;
    }
    return (signed short)((int)(entry & 0xFFFFU) - 32768);
}
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    g_gray_sparse_response_generation++;
    if(g_gray_sparse_response_generation == 0U)
    {
        memset(g_gray_sparse_response_cache, 0,
               sizeof(g_gray_sparse_response_cache));
        g_gray_sparse_response_generation = 1U;
    }
