...
#define GRAY_BEACON_MASK_ROW_BYTES     ((BEACON_IMAGE_W + 7) / 8)
...
static unsigned char g_gray_sparse_required[BEACON_IMAGE_H]
                                           [GRAY_BEACON_MASK_ROW_BYTES];
static unsigned short g_gray_sparse_bright_points[
    GRAY_BEACON_SPARSE_MAX_BRIGHT_PIXELS];
...
static int gray_lowest_set_bit(unsigned char bits)
{
#if defined(__GNUC__)
...
static void gray_mark_sparse_response_neighborhood(int x, int y)
{
    int first_x = x - 1;
    int byte_index = first_x >> 3;
    int shift = first_x & 7;
    unsigned short mask = (unsigned short)(7U << shift);
    int row;

    for(row = y - 1; row <= y + 1; row++)
    {
        g_gray_sparse_required[row][byte_index] |=
            (unsigned char)(mask & 0xFFU);
        if((mask >> 8) != 0U)
        {
            g_gray_sparse_required[row][byte_index + 1] |=
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    unsigned short bright_count = 0U;
    int byte_index;
    int x;
    int y;

    gray_build_integral_image(image);
    memset(g_gray_sparse_required, 0, sizeof(g_gray_sparse_required));
    for(y = 2; y <= BEACON_IMAGE_H - 3; y++)
    {
        for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
        {
            ...
            g_gray_sparse_bright_points[bright_count] = (unsigned short)(
                ((unsigned short)y << 8) | (unsigned short)x);
            bright_count++;
            gray_mark_sparse_response_neighborhood(x, y);
        }
    }
    for(y = 1; y <= BEACON_IMAGE_H - 2; y++)
    {
        for(byte_index = 0;
            byte_index < GRAY_BEACON_MASK_ROW_BYTES;
            byte_index++)
        {
            unsigned char bits = g_gray_sparse_required[y][byte_index];

            while(bits != 0U)
            {
                int bit = gray_lowest_set_bit(bits);

                x = byte_index * 8 + bit;
                if(x < BEACON_IMAGE_W)
                {
                    g_gray_sparse_response[y][x] =
                        gray_integral_response(x, y);
                }
                bits &= (unsigned char)(bits - 1U);
            }
        }
    }
    for(byte_index = 0; byte_index < bright_count; byte_index++)
    {
        unsigned short packed = g_gray_sparse_bright_points[byte_index];
        signed short response;

        x = (int)(packed & 0x00FFU);
        y = (int)(packed >> 8);
        response = g_gray_sparse_response[y][x];
            if((response <= 0) ||
            ...
            if((response < g_gray_sparse_response[y - 1][x - 1]) ||
               (response < g_gray_sparse_response[y][x - 1]) ||
               (response < g_gray_sparse_response[y + 1][x - 1]) ||
               (response < g_gray_sparse_response[y - 1][x]) ||
               (response < g_gray_sparse_response[y + 1][x]) ||
               (response < g_gray_sparse_response[y - 1][x + 1]) ||
               (response < g_gray_sparse_response[y][x + 1]) ||
               (response < g_gray_sparse_response[y + 1][x + 1]))
