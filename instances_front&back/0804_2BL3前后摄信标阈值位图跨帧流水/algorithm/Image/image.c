...
static unsigned int g_preprocessed_beacon_words[BEACON_IMAGE_H]
                                                     [IMAGE_BINARY_WORD_COUNT];
static unsigned char g_preprocess_beacon_threshold;
#else
static unsigned int g_preprocessed_beacon_words[BEACON_IMAGE_H]
                                                    [IMAGE_BINARY_WORD_COUNT];
#endif
...
#if IMAGE_USE_CM4_DSP
    g_preprocess_beacon_threshold = 0U;
#endif
}
    ...
    unsigned short *bright_pixel_count)
{
    const unsigned int bright_threshold4 =
        (unsigned int)GRAY_BEACON_MIN_PEAK_GRAY * 0x01010101U;
    const unsigned char predicted_beacon_threshold =
        g_preprocess_beacon_threshold;
    const unsigned int predicted_beacon_threshold4 =
        (unsigned int)predicted_beacon_threshold * 0x01010101U;
    unsigned int sum = 0U;
    unsigned int bright = 0U;
    float scene_mean;
    int y;
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        ...
        for(x = 0; x < BEACON_IMAGE_W; x += 4)
        {
            ...
            if((predicted_beacon_threshold != 0U) && (bits != 0U))
            {
                (void)__USUB8(pixels, predicted_beacon_threshold4);
                selected = __SEL(0x01010101U, 0U);
                beacon_bits |=
                    (((selected * 0x01020408U) >> 24) & bits) << shift;
            }
            ...
            if((shift == 28U) || (x == BEACON_IMAGE_W - 4))
            {
                ...
                if(predicted_beacon_threshold != 0U)
                {
                    beacon_words[word_index] = beacon_bits;
                }
                car_bits = 0U;
    ...
    scene_mean = (float)sum / (float)(BEACON_IMAGE_W * BEACON_IMAGE_H);
    {
        ...
        {
            if(beacon_threshold != predicted_beacon_threshold)
            {
                preprocess_beacon_words_dsp(image, beacon_threshold);
            }
            g_preprocess_beacon_threshold = beacon_threshold;
        }
        else
        {
            g_preprocess_beacon_threshold = 0U;
