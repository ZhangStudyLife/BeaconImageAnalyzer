...
static signed short gray_cached_integral_response_pruned(
    int x,
    int y,
    signed short minimum_response,
    unsigned char *pruned)
{
    unsigned int slot = (unsigned int)y & (GRAY_SPARSE_CACHE_ROWS - 1U);
    unsigned int word_index = (unsigned int)x >> 5;
    unsigned int bit = 1UL << ((unsigned int)x & 31U);
    ...
    if((g_gray_sparse_response_valid[slot][word_index] & bit) == 0U)
    {
        ...
        if((8 * (int)box3 <= (int)minimum_response) &&
           (pruned != 0))
        {
            *pruned = 1U;
            return 0;
        }
        g_gray_sparse_response[slot][x] = (signed short)(
            9 * (int)box3 - (int)gray_integral_box_sum(x, y, 4));
        g_gray_sparse_response_valid[slot][word_index] |= bit;
    }
    return g_gray_sparse_response[slot][x];
}
...
{
    signed short response;
    unsigned char pruned = 0U;

    if(gray_point_below_horizon(x, y) == 0U)
    {
        return;
    }
    response = (*count >= GRAY_BEACON_MAX_PEAKS) ?
        gray_cached_integral_response_pruned(
            x, y, peaks[GRAY_BEACON_MAX_PEAKS - 1].response, &pruned) :
        gray_cached_integral_response(x, y);
    if(pruned != 0U)
    {
        return;
    }
