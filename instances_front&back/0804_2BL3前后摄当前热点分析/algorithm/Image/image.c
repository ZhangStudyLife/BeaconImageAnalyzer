...
#include <math.h>
#include <string.h>
#if defined(IMAGE_HOST_PHASE_PROFILE)
#include <x86intrin.h>
#endif
...
enum
{
    ...
    PHASE_SCENE_MEAN,
    PHASE_PREPROCESS,
    PHASE_PREPROCESS_WORDS,
    PHASE_CAR_USE_WORDS,
    PHASE_CAR_BRIDGE,
    PHASE_CAR_SAVE_WORDS,
    PHASE_CAR_PRIMARY,
    PHASE_CAR_GRAY,
    PHASE_CAR_TEMPORAL,
    PHASE_BEACON_TOTAL,
...
static unsigned long long g_host_phase_profile[PHASE_COUNT];

void image_host_phase_profile_reset(void)
{
    memset(g_host_phase_profile, 0, sizeof(g_host_phase_profile));
}

void image_host_phase_profile_read(
    unsigned long long output[PHASE_COUNT])
{
    if(output != 0)
    {
        memcpy(output, g_host_phase_profile,
               sizeof(g_host_phase_profile));
    }
}
#endif
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    int slot;
#if defined(IMAGE_HOST_PHASE_PROFILE)
    unsigned long long phase_start = __rdtsc();
#endif

    gray_build_integral_image(image);
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_INTEGRAL_BUILD] +=
        __rdtsc() - phase_start;
    phase_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_PEAK_SCAN] += __rdtsc() - phase_start;
#endif
    return count;
}
...
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_PEAK_TOTAL] += __rdtsc() - phase_start;
    g_host_phase_profile[PHASE_PEAK_COUNT] += peak_count;
    phase_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_FEATURE] += __rdtsc() - phase_start;
    phase_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_COMPONENT] += __rdtsc() - phase_start;
    phase_start = __rdtsc();
#endif
    gray_store_candidates(
        image, candidates, candidate_count, scene_mean, result);
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_STORE] += __rdtsc() - phase_start;
    g_host_phase_profile[PHASE_CANDIDATE_COUNT] += candidate_count;
#endif
}
...
#if defined(IMAGE_HOST_PHASE_PROFILE)
    unsigned long long phase_start;
    unsigned long long phase_total_start;
#endif
...
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_SCENE_MEAN] += __rdtsc() - phase_start;
    phase_start = __rdtsc();
    phase_total_start = phase_start;
#endif
    preprocess_detection_words(image, scene_mean, &bright_pixel_count);
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_PREPROCESS_WORDS] +=
        __rdtsc() - phase_start;
    phase_start = __rdtsc();
#endif
    component_use_words(g_preprocessed_car_words);
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_CAR_USE_WORDS] += __rdtsc() - phase_start;
    phase_start = __rdtsc();
#endif
    bridge_upper_car_lamp_component_gaps();
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_CAR_BRIDGE] += __rdtsc() - phase_start;
    phase_start = __rdtsc();
#endif
    memcpy(g_preprocessed_car_words, g_component_source_words,
           sizeof(g_preprocessed_car_words));
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_CAR_SAVE_WORDS] += __rdtsc() - phase_start;
    g_host_phase_profile[PHASE_PREPROCESS] +=
        __rdtsc() - phase_total_start;
    phase_start = __rdtsc();
#endif
    has_lamp = find_car_lamp(image, &lamp);
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_CAR_PRIMARY] += __rdtsc() - phase_start;
#endif
    if(has_lamp == 0U)
    {
#if defined(IMAGE_HOST_PHASE_PROFILE)
...
#if defined(IMAGE_HOST_PHASE_PROFILE)
        g_host_phase_profile[PHASE_CAR_GRAY] += __rdtsc() - phase_start;
#endif
    }
    if(has_lamp == 0)
    {
...
#if defined(IMAGE_HOST_PHASE_PROFILE)
        g_host_phase_profile[PHASE_CAR_TEMPORAL] +=
            __rdtsc() - phase_start;
#endif
    }
    if(has_lamp == 0)
...
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_BEACON_TOTAL] += __rdtsc() - phase_start;
    phase_start = __rdtsc();
#endif
    update_temporal_car(result, (has_lamp != 0U) ? &lamp : 0);
    beacon_output_finalize(image, result);
#if defined(IMAGE_HOST_PHASE_PROFILE)
    g_host_phase_profile[PHASE_FINALIZE] += __rdtsc() - phase_start;
    g_host_phase_profile[PHASE_FRAME_COUNT]++;
#endif
}
