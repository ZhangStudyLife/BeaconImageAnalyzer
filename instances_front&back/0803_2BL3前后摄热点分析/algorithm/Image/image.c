...
static uint64_t g_host_profile_frames;
static uint64_t g_host_profile_setup_cycles;
static uint64_t g_host_profile_scene_cycles;
static uint64_t g_host_profile_car_threshold_cycles;
static uint64_t g_host_profile_car_primary_cycles;
static uint64_t g_host_profile_car_gray_cycles;
static uint64_t g_host_profile_car_temporal_cycles;
static uint64_t g_host_profile_beacon_cycles;
static uint64_t g_host_profile_finalize_cycles;
static uint64_t g_host_profile_gray_fallbacks;
static uint64_t g_host_profile_temporal_fallbacks;
static uint64_t g_host_profile_primary_hits;
static uint64_t g_host_profile_peak_scan_cycles;
static uint64_t g_host_profile_peak_feature_cycles;
static uint64_t g_host_profile_component_candidate_cycles;
static uint64_t g_host_profile_candidate_store_cycles;

void image_host_profile_reset(void)
{
    g_host_profile_frames = 0U;
    g_host_profile_setup_cycles = 0U;
    g_host_profile_scene_cycles = 0U;
    g_host_profile_car_threshold_cycles = 0U;
    g_host_profile_car_primary_cycles = 0U;
    g_host_profile_car_gray_cycles = 0U;
    g_host_profile_car_temporal_cycles = 0U;
    g_host_profile_beacon_cycles = 0U;
    g_host_profile_finalize_cycles = 0U;
    g_host_profile_gray_fallbacks = 0U;
    g_host_profile_temporal_fallbacks = 0U;
    g_host_profile_primary_hits = 0U;
    g_host_profile_peak_scan_cycles = 0U;
    g_host_profile_peak_feature_cycles = 0U;
    g_host_profile_component_candidate_cycles = 0U;
    g_host_profile_candidate_store_cycles = 0U;
}

void image_host_profile_read(uint64_t output[16])
{
    if(output == 0)
    {
        return;
    }
    output[0] = g_host_profile_frames;
    output[1] = g_host_profile_setup_cycles;
    output[2] = g_host_profile_scene_cycles;
    output[3] = g_host_profile_car_threshold_cycles;
    output[4] = g_host_profile_car_primary_cycles;
    output[5] = g_host_profile_car_gray_cycles;
    output[6] = g_host_profile_car_temporal_cycles;
    output[7] = g_host_profile_beacon_cycles;
    output[8] = g_host_profile_finalize_cycles;
    output[9] = g_host_profile_gray_fallbacks;
    output[10] = g_host_profile_temporal_fallbacks;
    output[11] = g_host_profile_primary_hits;
    output[12] = g_host_profile_peak_scan_cycles;
    output[13] = g_host_profile_peak_feature_cycles;
    output[14] = g_host_profile_component_candidate_cycles;
    output[15] = g_host_profile_candidate_store_cycles;
}
...
static void gray_horizontal_box_sum(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    int y,
    int radius,
    unsigned short output[BEACON_IMAGE_W])
{
    ...
    {
        sum += image[y][gray_reflect_index(x, BEACON_IMAGE_W)];
    }
    output[0] = (unsigned short)sum;

    for(x = 1; x < BEACON_IMAGE_W; x++)
    {
        sum += image[y][gray_reflect_index(x + radius, BEACON_IMAGE_W)];
        sum -= image[y][gray_reflect_index(x - radius - 1, BEACON_IMAGE_W)];
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    {
        int row = gray_reflect_index(index - 1, BEACON_IMAGE_H);
        gray_horizontal_box_sum(image, row, 1, g_gray_box3_rows[index]);
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
    ...
    {
        int row = gray_reflect_index(index - 4, BEACON_IMAGE_H);
        gray_horizontal_box_sum(image, row, 4, g_gray_box9_rows[index]);
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        ...
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
            int response = 9 * (int)g_gray_box3_sum[x] -
                           (int)g_gray_box9_sum[x];
        ...
        if((y >= 3) && (y <= BEACON_IMAGE_H - 2))
        {
            ...
            for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
            {
                ...
                if((gray_point_below_horizon(x, center_y) == 0U) ||
                   (response <= 0) ||
                ...
                for(dx = -1; dx <= 1; dx++)
                {
                    if((response < g_gray_response_rows[top_slot][x + dx]) ||
                       (response < g_gray_response_rows[center_slot][x + dx]) ||
                       (response < g_gray_response_rows[bottom_slot][x + dx]))
                    {
                        break;
                    }
                }
        ...
        gray_horizontal_box_sum(
            image, gray_reflect_index(y + 2, BEACON_IMAGE_H), 1,
            g_gray_box3_rows[index]);
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
            g_gray_box3_sum[x] += g_gray_box3_rows[index][x];
        }
        ...
        gray_horizontal_box_sum(
            image, gray_reflect_index(y + 5, BEACON_IMAGE_H), 4,
            g_gray_box9_rows[index]);
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
            g_gray_box9_sum[x] += g_gray_box9_rows[index][x];
        }
    ...
    beacon_result_t *result)
{
    ...
    g_host_profile_peak_scan_cycles += __rdtsc() - profile_start;
    ...
    g_host_profile_peak_feature_cycles += __rdtsc() - profile_start;

    profile_start = __rdtsc();
    gray_add_component_candidates(
        image, lamp, temporal_lamp, scene_mean,
        candidates, &candidate_count);
    g_host_profile_component_candidate_cycles += __rdtsc() - profile_start;

    profile_start = __rdtsc();
    gray_store_candidates(
        image, candidates, candidate_count, scene_mean, result);
    g_host_profile_candidate_store_cycles += __rdtsc() - profile_start;
}
    ...
    beacon_result_t *result)
{
    ...
    g_host_profile_setup_cycles += __rdtsc() - profile_start;

    profile_start = __rdtsc();
    scene_mean = gray_image_mean(image);
    g_host_profile_scene_cycles += __rdtsc() - profile_start;

    profile_start = __rdtsc();
    threshold_car_lamp_image(image);
    g_host_profile_car_threshold_cycles += __rdtsc() - profile_start;

    profile_start = __rdtsc();
    has_lamp = find_car_lamp(image, &lamp);
    g_host_profile_car_primary_cycles += __rdtsc() - profile_start;
    if(has_lamp != 0U)
    {
        g_host_profile_primary_hits++;
    }
    if(has_lamp == 0U)
    {
        g_host_profile_gray_fallbacks++;
        profile_start = __rdtsc();
        has_lamp = find_gray_car_lamp(image, scene_mean, &lamp);
        g_host_profile_car_gray_cycles += __rdtsc() - profile_start;
    }
    if(has_lamp == 0)
    {
        g_host_profile_temporal_fallbacks++;
        profile_start = __rdtsc();
        has_lamp = find_temporal_car_lamp(image, &lamp);
        g_host_profile_car_temporal_cycles += __rdtsc() - profile_start;
    }
    if(has_lamp == 0)
    {
        memset(&lamp, 0, sizeof(lamp));
    ...
    g_host_profile_finalize_cycles += __rdtsc() - profile_start;

    profile_start = __rdtsc();
    find_beacons(image, &lamp, &temporal_lamp, scene_mean, result);
    g_host_profile_beacon_cycles += __rdtsc() - profile_start;

    profile_start = __rdtsc();
    update_temporal_car(result);
    beacon_output_finalize(image, result);
    g_host_profile_finalize_cycles += __rdtsc() - profile_start;
    g_host_profile_frames++;
}
