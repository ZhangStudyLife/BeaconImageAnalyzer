...
static float gray_prepare_frame(
...
static unsigned short g_gray_bright_points[
    GRAY_BEACON_SPARSE_MAX_BRIGHT_PIXELS + 1];
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    for(point_index = 0U;
        point_index < bright_pixel_count;
        point_index++)
    {
        unsigned short packed = g_gray_bright_points[point_index];
        int x = (int)(packed & 0x00FFU);
        int y = (int)(packed >> 8);
        signed short response;
    ...
    unsigned short *bright_pixel_count)
{
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        ...
        unsigned char *binary = g_binary[y];
        unsigned char threshold = CAR_LAMP_BINARY_THRESHOLD;
        int x;
    ...
    beacon_result_t *result)
{
    ...
    scene_mean = gray_prepare_frame(image, &bright_pixel_count);
