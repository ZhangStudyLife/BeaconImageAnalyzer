...
static unsigned char g_gray_seed_peak_used[(IMAGE_QUEUE_SIZE + 7) / 8];
...
static void gray_prepare_integral_response_cache(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W])
{
...
static inline void gray_try_insert_integral_peak(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    int x,
    int y,
    const component_t *lamp,
    ...
    unsigned char used[(IMAGE_QUEUE_SIZE + 7) / 8],
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS],
    unsigned char *count)
{
    ...
    if((x < 2) || (x > BEACON_IMAGE_W - 3) ||
       (y < 2) || (y > BEACON_IMAGE_H - 3) ||
       (image[y][x] < GRAY_BEACON_MIN_PEAK_GRAY))
    {
        return;
    }
    pixel_index = (unsigned int)y * BEACON_IMAGE_W + (unsigned int)x;
    byte_index = pixel_index >> 3;
    bit = (unsigned char)(1U << (pixel_index & 7U));
    if((used != 0) && ((used[byte_index] & bit) != 0U))
    {
        return;
    }
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    gray_prepare_integral_response_cache(image);
    for(y = 2; y <= BEACON_IMAGE_H - 3; y++)
    {
        for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
        {
            ...
            gray_try_insert_integral_peak(
                image, x, y, lamp, temporal_lamp,
                lamp_cos, lamp_sin,
...
static unsigned char gray_raw_local_maximum(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    int x,
    int y)
{
    unsigned char center = image[y][x];
    unsigned char greater = 0U;
    int dx;
    int dy;
...
static unsigned char gray_find_seeded_integral_peaks(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    const component_t *lamp,
    const component_t *temporal_lamp,
    float lamp_cos,
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    gray_prepare_integral_response_cache(image);
    memset(g_gray_seed_peak_used, 0, sizeof(g_gray_seed_peak_used));
    for(y = 2; y <= BEACON_IMAGE_H - 3; y++)
    {
        for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
        {
            signed short best_response = -32768;
            int best_x = x;
            int best_y = y;
            int dx;
            int dy;
            ...
            if((image[y][x] < GRAY_BEACON_MIN_PEAK_GRAY) ||
               (gray_raw_local_maximum(image, x, y) == 0U))
            {
                continue;
            }
            for(dy = -1; dy <= 1; dy++)
            {
                for(dx = -1; dx <= 1; dx++)
                {
                    int candidate_x = x + dx;
                    int candidate_y = y + dy;
                    signed short response;

                    if((candidate_x < 2) ||
                       (candidate_x > BEACON_IMAGE_W - 3) ||
                       (candidate_y < 2) ||
                       (candidate_y > BEACON_IMAGE_H - 3) ||
                       (image[candidate_y][candidate_x] <
                        GRAY_BEACON_MIN_PEAK_GRAY))
                    {
                        continue;
                    }
                    response = gray_cached_integral_response(
            ...
            gray_try_insert_integral_peak(
                ...
                lamp_cos, lamp_sin,
                temporal_lamp_cos, temporal_lamp_sin,
                g_gray_seed_peak_used, peaks, &count);
        }
    }
    return count;
}
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    if(bright_pixel_count <= BEACON_IMAGE_W * BEACON_IMAGE_H)
    {
        return gray_find_seeded_integral_peaks(
            image, lamp, temporal_lamp,
            lamp_cos, lamp_sin,
            temporal_lamp_cos, temporal_lamp_sin,
            peaks);
