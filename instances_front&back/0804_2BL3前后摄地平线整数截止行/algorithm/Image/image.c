...
#endif
static signed short g_gray_horizon_cutoff_y[BEACON_IMAGE_W];
static unsigned char g_gray_horizon_cutoff_ready;
...
static void gray_prepare_horizon_cutoff(unsigned short bright_pixel_count)
{
    int x;

    g_gray_horizon_cutoff_ready = 0U;
    if((bright_pixel_count <= GRAY_BEACON_DIRECT_MAX_BRIGHT_PIXELS) ||
    ...
    for(x = 0; x < BEACON_IMAGE_W; x++)
    {
        ...
        {
            g_gray_horizon_cutoff_y[x] = -32768;
            continue;
        }
        threshold = g_image_horizon_y[x] -
                    BEACON_HORIZON_TOLERANCE_PX;
        if(threshold != threshold)
        {
            cutoff = BEACON_IMAGE_H;
        }
        else if(threshold <= 0.0f)
        {
            cutoff = 0;
        }
        else if(threshold > (float)(BEACON_IMAGE_H - 1))
        {
            cutoff = BEACON_IMAGE_H;
        }
        else
        {
            ...
            if((float)cutoff < threshold)
            {
                cutoff++;
            }
        }
        g_gray_horizon_cutoff_y[x] = (signed short)cutoff;
    }
    g_gray_horizon_cutoff_ready = 1U;
}

static unsigned char gray_point_below_horizon(int x, int y)
{
    if((g_gray_horizon_cutoff_ready != 0U) &&
       (x >= 0) && (x < BEACON_IMAGE_W))
    {
        return (y >= (int)g_gray_horizon_cutoff_y[x]) ? 1U : 0U;
    }
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    gray_prepare_horizon_cutoff(bright_pixel_count);
