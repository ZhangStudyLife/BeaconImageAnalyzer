...
static int gray_first_candidate_row(void)
{
    float minimum_y = (float)BEACON_IMAGE_H;
    int candidate_y;
    int x;
    ...
    for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
    {
        ...
        allowed_y = g_image_horizon_y[x] - BEACON_HORIZON_TOLERANCE_PX;
        if(allowed_y < minimum_y)
        {
            minimum_y = allowed_y;
        }
    }
    candidate_y = (int)minimum_y;
    if((float)candidate_y < minimum_y)
    {
        candidate_y++;
    }
    if(candidate_y < 2)
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    for(y = first_candidate_y; y <= BEACON_IMAGE_H - 3; y++)
    {
        for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
        {
            signed short response;
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    first_candidate_y = gray_first_candidate_row();
    if(first_candidate_y > BEACON_IMAGE_H - 3)
    {
        return 0U;
    }
    ...
    gray_initialize_vertical_box_sums(image, first_candidate_y - 1);

    for(y = first_candidate_y - 1; y <= BEACON_IMAGE_H - 2; y++)
    {
        int response_slot = y % 3;

        gray_calculate_response_row(response_slot);

        if(y >= first_candidate_y + 1)
        {
            int center_y = y - 1;
            int top_slot = (y - 2) % 3;
