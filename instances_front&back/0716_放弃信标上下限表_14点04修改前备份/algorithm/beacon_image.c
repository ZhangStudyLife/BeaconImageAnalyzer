...
static int perspective_min_beacon_area(const component_t *comp)
{
    ...
    if(center_y <= BEACON_PERSPECTIVE_MIN_AREA_START_Y)
    {
        return 0;
    }

    min_area = BEACON_MIN_COMPONENT_AREA +
        (center_y - BEACON_PERSPECTIVE_MIN_AREA_START_Y +
    ...
    beacon_result_t *result)
{
    ...
    if(is_side_edge == 0U)
    {
        int perspective_min_area = perspective_min_beacon_area(comp);

        if(perspective_min_area > min_area)
        {
            min_area = perspective_min_area;
        }
    }
    ...
    beacon_result_t *result)
{
    ...
    find_weak_center_beacons(image, lamp, temporal_lamp, result);
}
    ...
    beacon_result_t *result)
{
    ...
    suppress_bad_shape_beacons(image, result);
