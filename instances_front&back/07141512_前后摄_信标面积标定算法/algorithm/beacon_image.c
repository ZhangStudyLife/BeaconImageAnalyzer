...
#define BEACON_TOP_EDGE_MAX_AREA  74
...
static const uint8 g_front_beacon_area_upper[BEACON_AREA_GATE_Y_COUNT][BEACON_AREA_GATE_X_COUNT] =
{
    ...
    {  0U,   0U,   0U,  41U,  46U,  20U,   0U,   0U,   0U},
    {  0U,   0U,  27U,  74U,  72U,  31U,  25U,   0U,   0U},
    {  0U,   0U,  56U, 102U,  62U,  55U,  25U,  51U,   0U},
    {  0U,  60U,  80U, 113U, 126U, 116U, 157U,  35U,   0U},
...
static const uint8 g_front_beacon_area_lower[BEACON_AREA_GATE_Y_COUNT][BEACON_AREA_GATE_X_COUNT] =
{
    ...
    { 8U, 15U, 32U, 36U, 41U, 38U,  7U, 10U, 0U},
...
static void write_temporal_car(
    const temporal_track_t *track,
    const beacon_rect_t *measurement,
    beacon_result_t *result)
{
    /* 当前帧有可信车灯时直接输出测量值，仅丢帧时使用时序预测。 */
    result->temporal_car_lamps[0].cx =
        (measurement != 0) ? measurement->cx : track->x;
    result->temporal_car_lamps[0].cy =
        (measurement != 0) ? measurement->cy : track->y;
    result->temporal_car_lamps[0].width =
        (measurement != 0) ? measurement->width : track->width;
    result->temporal_car_lamps[0].length =
        (measurement != 0) ? measurement->length : track->length;
    result->temporal_car_lamps[0].angle =
        (measurement != 0) ? measurement->angle : track->angle;
...
static void update_temporal_car(beacon_result_t *result)
{
    ...
    if(measurement == 0)
    {
           ...
           (g_car_track.misses < temporal_car_max_misses(&g_car_track)))
        {
            ...
            g_car_track.misses++;
            write_temporal_car(&g_car_track, 0, result);
            return;
        }
    ...
    if(g_car_track.active == 0U)
    {
        ...
        write_temporal_car(&g_car_track, measurement, result);
        return;
    }
