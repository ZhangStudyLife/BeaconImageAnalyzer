...
static void component_calculate_direction(component_t *comp)
{
...
#endif
    axis_length = sqrtf(comp->angle_x * comp->angle_x +
                        comp->angle_y * comp->angle_y);
    if(axis_length == 0.0f)
    {
        comp->direction_cos = 1.0f;
        comp->direction_sin = 0.0f;
        comp->direction_valid = 1U;
        return;
    }

    if(comp->angle_x >= 0.0f)
    {
        vector_x = axis_length + comp->angle_x;
        vector_y = comp->angle_y;
    }
    else
    {
        vector_x = comp->angle_y;
        vector_y = axis_length - comp->angle_x;
    }
    vector_length = sqrtf(vector_x * vector_x + vector_y * vector_y);
    ...
    {
        comp->direction_cos = vector_x / vector_length;
        comp->direction_sin = vector_y / vector_length;
    }
    comp->direction_valid = 1U;
}
    ...
    component_t *comp)
{
    ...
    if(((comp->min_x <= 0) || (comp->max_x >= BEACON_IMAGE_W - 1)) &&
       (fabsf(comp->direction_sin) >= 0.8660254f))
    {
        return 0U;
    }
    cos_a = comp->direction_cos;
...
static void write_car_lamp(component_t *lamp, beacon_result_t *result)
{
    ...
    component_calculate_angle(lamp);
