...
static unsigned int g_gray_vertical39_sum[
    BEACON_IMAGE_W + GRAY_BEACON_BOX_PAD * 2];
static unsigned int g_gray_bright_words[BEACON_IMAGE_H]
    ...
    unsigned int *remaining_bits)
{
    ...
    {
        if(*word_index >= IMAGE_BINARY_WORD_COUNT)
        {
            return -1;
        }
        *remaining_bits = g_gray_bright_words[y][*word_index];
    ...
    {
        unsigned int bit = image_count_trailing_zeros(*remaining_bits);
        int x = *word_index * 32 + (int)bit;
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    for(y = 2; y <= BEACON_IMAGE_H - 3; y++)
    {
        ...
        {
            int word_index = 0;
            unsigned int remaining_bits = 0U;

            while((x = gray_next_bright_x(
                       y, &word_index, &remaining_bits)) >= 0)
            {
#else
        ...
        for(y = 0; y < BEACON_IMAGE_H; y++)
        {
            ...
            for(x = 0; x < BEACON_IMAGE_W; x += 4)
            {
                ...
                if(((x & 31) == 28) ||
                   (x + 4 >= BEACON_IMAGE_W))
                {
                    g_gray_bright_words[y][word_index++] = word_bits;
                    word_bits = 0U;
                }
            }
        }
