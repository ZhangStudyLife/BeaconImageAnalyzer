...
static unsigned int (*g_component_source_words)
    [IMAGE_BINARY_WORD_COUNT] = g_component_temporary_words;
...
static void component_use_words(
    unsigned int words[BEACON_IMAGE_H][IMAGE_BINARY_WORD_COUNT])
{
    g_component_source_words = words;
    memcpy(g_component_remaining_words, words,
           sizeof(g_component_remaining_words));
}

static void component_use_internal_words(void)
{
    g_component_source_words = g_component_temporary_words;
}
