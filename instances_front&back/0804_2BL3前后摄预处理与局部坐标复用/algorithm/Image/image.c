    ...
    unsigned char point_source_mode)
{
    ...
    unsigned char patch_x[GRAY_BEACON_PATCH_RADIUS * 2 + 1];
    unsigned char patch_y[GRAY_BEACON_PATCH_RADIUS * 2 + 1];
    ...
    {
        patch_x[dx + GRAY_BEACON_PATCH_RADIUS] =
            (unsigned char)gray_reflect_index(
                center_x + dx, BEACON_IMAGE_W);
    }
    for(dy = -GRAY_BEACON_PATCH_RADIUS;
        dy <= GRAY_BEACON_PATCH_RADIUS; dy++)
    {
        patch_y[dy + GRAY_BEACON_PATCH_RADIUS] =
            (unsigned char)gray_reflect_index(
    ...
    {
        const unsigned char *row =
            image[patch_y[dy + GRAY_BEACON_PATCH_RADIUS]];
        ...
        for(dx = -half_width; dx <= half_width; dx++)
        {
            ...
            unsigned char pixel =
                row[patch_x[dx + GRAY_BEACON_PATCH_RADIUS]];
