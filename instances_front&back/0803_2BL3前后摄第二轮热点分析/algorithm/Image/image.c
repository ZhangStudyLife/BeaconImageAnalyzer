...
static unsigned short g_gray_vertical3_sum[BEACON_IMAGE_W];
static unsigned short g_gray_vertical9_sum[BEACON_IMAGE_W];
static signed short g_gray_response_rows[3][BEACON_IMAGE_W];

static uint64_t g_host_profile[16];
    ...
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W])
{
    ...
    for(offset = -4; offset <= 4; offset++)
    {
        const unsigned char *row =
            image[gray_reflect_index(offset, BEACON_IMAGE_H)];

        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
            g_gray_vertical9_sum[x] += row[x];
            if((offset >= -1) && (offset <= 1))
            {
                g_gray_vertical3_sum[x] += row[x];
            }
        }
    }
}
...
static void gray_calculate_response_row(int response_slot)
{
    ...
    for(x = -4; x <= 4; x++)
    {
        int column = gray_reflect_index(x, BEACON_IMAGE_W);

        horizontal9 += g_gray_vertical9_sum[column];
        if((x >= -1) && (x <= 1))
        {
            horizontal3 += g_gray_vertical3_sum[column];
        }
    }
    ...
    {
        horizontal3 += g_gray_vertical3_sum[
            gray_reflect_index(x + 1, BEACON_IMAGE_W)];
        horizontal3 -= g_gray_vertical3_sum[
            gray_reflect_index(x - 2, BEACON_IMAGE_W)];
        horizontal9 += g_gray_vertical9_sum[
            gray_reflect_index(x + 4, BEACON_IMAGE_W)];
        horizontal9 -= g_gray_vertical9_sum[
            gray_reflect_index(x - 5, BEACON_IMAGE_W)];
    ...
    int y)
{
    ...
    for(x = 0; x < BEACON_IMAGE_W; x++)
    {
        g_gray_vertical3_sum[x] = (unsigned short)(
            (int)g_gray_vertical3_sum[x] + enter3[x] - leave3[x]);
        g_gray_vertical9_sum[x] = (unsigned short)(
            (int)g_gray_vertical9_sum[x] + enter9[x] - leave9[x]);
    }
}
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    gray_initialize_vertical_box_sums(image);
    ...
    beacon_result_t *result)
{
    ...
    g_host_profile[12] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
    ...
    beacon_result_t *result)
{
    ...
    g_host_profile[1] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
    scene_mean = gray_image_mean(image);
    g_host_profile[2] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
    threshold_car_lamp_image(image);
    g_host_profile[3] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
    has_lamp = find_car_lamp(image, &lamp);
    g_host_profile[4] += __rdtsc() - profile_start;
    if(has_lamp == 0U)
    {
        g_host_profile[9]++;
        profile_start = __rdtsc();
        has_lamp = find_gray_car_lamp(image, scene_mean, &lamp);
        g_host_profile[5] += __rdtsc() - profile_start;
    }
    if(has_lamp == 0)
    {
        g_host_profile[10]++;
        ...
        g_host_profile[6] += __rdtsc() - profile_start;
    }
    else
    {
        g_host_profile[11]++;
    ...
    g_host_profile[8] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
    find_beacons(image, &lamp, &temporal_lamp, scene_mean, result);
    g_host_profile[7] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
