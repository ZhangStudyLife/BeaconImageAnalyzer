...
typedef union
{
    signed short sparse_response[BEACON_IMAGE_H][BEACON_IMAGE_W];
    ...
    unsigned char threshold)
{
    ...
    memset(g_binary_foreground_words, 0,
           sizeof(g_binary_foreground_words));
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        ...
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
            unsigned char foreground =
                (image[y][x] >= threshold) ?
                IMAGE_BINARY_FOREGROUND : 0U;

            g_binary[y][x] = foreground;
            if(foreground != 0U)
            {
                g_binary_foreground_words[y][x >> 5] |=
                    1UL << (x & 31);
    ...
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W])
{
    ...
    for(padded_y = 0; padded_y < GRAY_BEACON_PADDED_H; padded_y++)
    {
        ...
        {
            row_sum += source[x];
            current[output_x] = previous[output_x] + row_sum;
            output_x++;
        }
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
