...
#define g_binary_foreground_words (g_gray_stage_scratch.foreground_words)
...
enum
{
    PROFILE_FRAMES = 0,
    PROFILE_SCENE,
    PROFILE_CAR_THRESHOLD,
    PROFILE_CAR_PRIMARY,
    PROFILE_CAR_GRAY,
    PROFILE_CAR_TEMPORAL,
    PROFILE_BEACON,
    PROFILE_FINALIZE,
    PROFILE_PEAK,
    PROFILE_FEATURE,
...
static uint64_t g_host_profile[PROFILE_COUNT];
...
void image_host_profile_read(uint64_t output[PROFILE_COUNT])
{
    if(output != 0)
    {
        memcpy(output, g_host_profile, sizeof(g_host_profile));
    }
}
#endif
...
#if IMAGE_USE_CM4_DSP
    {
        ...
        for(x = 0; x < BEACON_IMAGE_W; x += 4)
        {
            ...
            *(unsigned int *)&destination[x] = selected;
            bits = (selected & 0x00000001U) |
                   ((selected >> 7) & 0x00000002U) |
                   ((selected >> 14) & 0x00000004U) |
                   ((selected >> 21) & 0x00000008U);
    ...
    unsigned short *tail)
{
    ...
    if((tail == 0) || (*tail >= IMAGE_SPAN_QUEUE_SIZE) ||
       ...
       (g_binary[y][seed_x] != IMAGE_BINARY_FOREGROUND))
    {
        return -1;
    }
    ...
    left = seed_x;
    while((left > 0) &&
          (g_binary[y][left - 1] == IMAGE_BINARY_FOREGROUND))
    {
        left--;
    }
    right = seed_x;
    while((right + 1 < BEACON_IMAGE_W) &&
          (g_binary[y][right + 1] == IMAGE_BINARY_FOREGROUND))
    {
        right++;
    }
    for(x = left; x <= right; x++)
    {
        g_binary[y][x] = IMAGE_BINARY_VISITED;
    }
    ...
    int shape_max_area)
{
    ...
    while(head < tail)
    {
        ...
        for(neighbor_index = 0; neighbor_index < 2; neighbor_index++)
        {
            ...
            while(scan_x <= scan_right)
            {
                int run_right;

                if(g_binary[neighbor_y][scan_x] !=
                   IMAGE_BINARY_FOREGROUND)
                {
                    scan_x++;
                    continue;
                }
                ...
                scan_x = (run_right >= scan_x) ?
                         run_right + 1 : scan_x + 1;
            }
        }
    }
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
            ...
            word_index++)
        {
            ...
            while(foreground != 0U)
            {
                ...
                if((x >= BEACON_IMAGE_W) ||
                   (g_binary[y][x] != IMAGE_BINARY_FOREGROUND))
                {
                    continue;
                }
                comp = grow_component(
                    (unsigned char)x, (unsigned char)y,
                    CAR_LAMP_MIN_AREA, CAR_LAMP_MAX_AREA);
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    {
#if defined(IMAGE_HOST_PROFILE)
        g_host_profile[PROFILE_SPARSE_COUNT]++;
#endif
...
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_DENSE_COUNT]++;
#endif
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
            ...
            word_index++)
        {
            ...
            while(foreground != 0U)
            {
                ...
                {
                    (void)grow_component(
                        (unsigned char)x, (unsigned char)y, 0, -1);
                    continue;
                }
                comp = grow_component(
                    (unsigned char)x, (unsigned char)y,
...
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_PEAK] += __rdtsc() - profile_start;
    g_host_profile[PROFILE_PEAK_COUNT] += peak_count;
    profile_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_FEATURE] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
#endif
...
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_COMPONENT] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
#endif
    gray_store_candidates(
        image, candidates, candidate_count, scene_mean, result);
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_STORE] += __rdtsc() - profile_start;
    g_host_profile[PROFILE_CANDIDATE_COUNT] += candidate_count;
#endif
}
            ...
            word_index++)
        {
            ...
            while(foreground != 0U)
            {
                ...
                if((candidate_x < min_x) || (candidate_x > max_x) ||
                   ...
                   (g_binary[y][candidate_x] != IMAGE_BINARY_FOREGROUND))
                {
                    continue;
                }
                comp = grow_component(
...
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_SCENE] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
#endif
    threshold_car_lamp_image(image);
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_CAR_THRESHOLD] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
#endif
    has_lamp = find_car_lamp(image, &lamp);
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_CAR_PRIMARY] += __rdtsc() - profile_start;
#endif
    if(has_lamp == 0U)
    {
#if defined(IMAGE_HOST_PROFILE)
...
#if defined(IMAGE_HOST_PROFILE)
        g_host_profile[PROFILE_CAR_GRAY] += __rdtsc() - profile_start;
#endif
    }
    if(has_lamp == 0)
    {
...
#if defined(IMAGE_HOST_PROFILE)
        g_host_profile[PROFILE_CAR_TEMPORAL] +=
...
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_BEACON] += __rdtsc() - profile_start;
    profile_start = __rdtsc();
#endif
    update_temporal_car(result);
    beacon_output_finalize(image, result);
#if defined(IMAGE_HOST_PROFILE)
    g_host_profile[PROFILE_FINALIZE] += __rdtsc() - profile_start;
    g_host_profile[PROFILE_FRAMES]++;
#endif
}
