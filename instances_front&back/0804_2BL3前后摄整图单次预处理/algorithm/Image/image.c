...
#define GRAY_BEACON_DARK_COMPONENT_THRESHOLD   80U
#define GRAY_BEACON_BRIGHT_COMPONENT_THRESHOLD 120U
...
static unsigned int g_preprocessed_beacon_bright_words[BEACON_IMAGE_H]
                                                          [IMAGE_BINARY_WORD_COUNT];
static const unsigned int (*g_preprocessed_beacon_selected_words)
    [IMAGE_BINARY_WORD_COUNT] = g_preprocessed_beacon_words;
    ...
    int *maximum_area)
{
    if(scene_mean < GRAY_BEACON_DARK_SCENE_MEAN)
    {
        if(threshold != 0) *threshold =
            GRAY_BEACON_DARK_COMPONENT_THRESHOLD;
    ...
    {
        if(threshold != 0) *threshold =
            GRAY_BEACON_BRIGHT_COMPONENT_THRESHOLD;
    ...
    unsigned short *bright_pixel_count)
{
    ...
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        ...
        unsigned int *dark_beacon_words = g_preprocessed_beacon_words[y];
        unsigned int *bright_beacon_words =
        ...
        memset(car_words, 0,
               sizeof(unsigned int) * IMAGE_BINARY_WORD_COUNT);
        memset(dark_beacon_words, 0,
               sizeof(unsigned int) * IMAGE_BINARY_WORD_COUNT);
        memset(bright_beacon_words, 0,
               sizeof(unsigned int) * IMAGE_BINARY_WORD_COUNT);
#if IMAGE_USE_CM4_DSP
        {
            ...
            unsigned int dark_beacon_threshold4 =
                GRAY_BEACON_DARK_COMPONENT_THRESHOLD * 0x01010101U;
            unsigned int bright_beacon_threshold4 =
                GRAY_BEACON_BRIGHT_COMPONENT_THRESHOLD * 0x01010101U;
            ...
            for(x = 0; x < BEACON_IMAGE_W; x += 4)
            {
                ...
                gray_sum = __USADA8(pixels, 0U, gray_sum);
                ...
                (void)__USUB8(pixels, dark_beacon_threshold4);
                selected = __SEL(0x01010101U, 0U);
                bits = (selected * 0x01020408U) >> 24;
                dark_beacon_words[x >> 5] |= bits << (x & 31);

                (void)__USUB8(pixels, bright_beacon_threshold4);
                selected = __SEL(0x01010101U, 0U);
                bits = (selected * 0x01020408U) >> 24;
                bright_beacon_words[x >> 5] |= bits << (x & 31);
            }
        }
#else
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
            ...
            if(gray >= GRAY_BEACON_DARK_COMPONENT_THRESHOLD)
            {
                dark_beacon_words[x >> 5] |= 1UL << (x & 31);
            }
            if(gray >= GRAY_BEACON_BRIGHT_COMPONENT_THRESHOLD)
            {
                bright_beacon_words[x >> 5] |= 1UL << (x & 31);
            }
        }
#endif
    }
    ...
    g_preprocessed_beacon_selected_words =
        (mean >= GRAY_BEACON_BRIGHT_SCENE_MEAN) ?
        g_preprocessed_beacon_bright_words : g_preprocessed_beacon_words;
    if(scene_mean != 0)
    {
        *scene_mean = mean;
    }
    ...
    unsigned char *count)
{
    ...
    component_use_words(g_preprocessed_beacon_selected_words);
