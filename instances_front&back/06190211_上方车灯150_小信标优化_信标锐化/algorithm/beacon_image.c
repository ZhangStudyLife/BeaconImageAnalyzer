...
#define BEACON_EDGE_THRESHOLD     65
#define BEACON_SHARPEN_SHIFT      1
...
static unsigned char sharpen_beacon_pixel(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    int x,
    int y)
{
    ...
    sharp = value + ((value - mean) >> BEACON_SHARPEN_SHIFT);
    if (sharp < 0) sharp = 0;
    if (sharp > 255) sharp = 255;
    return (unsigned char)sharp;
}
    ...
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W])
{
    ...
    for (y = 0; y < BEACON_IMAGE_H; y++)
    {
        for (x = 0; x < BEACON_IMAGE_W; x++)
        {
            ...
            g_binary[y][x] = (sharpen_beacon_pixel(image, x, y) >= threshold) ?
                             255 : 0;
        }
    }
    memset(g_visit, 0, sizeof(g_visit));
