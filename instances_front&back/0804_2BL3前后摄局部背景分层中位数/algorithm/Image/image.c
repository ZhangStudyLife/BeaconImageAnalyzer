    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    for(y = 2; y <= BEACON_IMAGE_H - 3; y++)
    {
        ...
        for(x = 0; x < BEACON_IMAGE_W; x += 4)
        {
            unsigned int pixels = *(const unsigned int *)&image[y][x];
            unsigned int selected;
            unsigned int bits;
    ...
    unsigned char point_source_mode)
{
    ...
    unsigned char ring_pixels[80];
    unsigned char coarse_histogram[16];
    unsigned char low_histogram[16];
    unsigned char high_histogram[16];
    ...
    memset(coarse_histogram, 0, sizeof(coarse_histogram));
        ...
        dy <= GRAY_BEACON_PATCH_RADIUS; dy++)
    {
        ...
        for(dx = -half_width; dx <= half_width; dx++)
        {
            ...
            {
                ring_pixels[ring_count] = pixel;
                coarse_histogram[pixel >> 4]++;
                ring_count++;
            }
            if(radius2 <= 4)
            {
    ...
    {
        int next = cumulative + coarse_histogram[value];

        if((low_found == 0) && (next > low_rank))
        {
            low_bin = value;
            low_before = cumulative;
            low_found = 1;
        }
        if(next > high_rank)
        {
            high_bin = value;
            high_before = cumulative;
            break;
        }
        cumulative = next;
    }
    memset(low_histogram, 0, sizeof(low_histogram));
    memset(high_histogram, 0, sizeof(high_histogram));
    for(value = 0; value < ring_count; value++)
    {
        unsigned char pixel = ring_pixels[value];
        int bin = pixel >> 4;

        if(bin == low_bin)
        {
            low_histogram[pixel & 0x0FU]++;
        }
        if(bin == high_bin)
        {
            high_histogram[pixel & 0x0FU]++;
        }
    }
    cumulative = low_before;
    for(value = 0; value < 16; value++)
    {
        cumulative += low_histogram[value];
    ...
    cumulative = high_before;
    for(value = 0; value < 16; value++)
    {
        cumulative += high_histogram[value];
        if(cumulative > high_rank)
        {
            high = (high_bin << 4) | value;
            break;
        }
    }
