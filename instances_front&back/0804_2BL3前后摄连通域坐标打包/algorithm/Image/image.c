...
typedef union
{
    struct
    {
        ...
        unsigned short queue_xy[IMAGE_QUEUE_SIZE];
    } components;
...
#define g_queue_xy (g_image_scratch.components.queue_xy)
    ...
    int shape_max_area)
{
    ...
    comp.min_y = start_y;
    comp.max_y = start_y;
    g_queue_xy[tail] = (unsigned short)(
        ((unsigned short)start_y << 8) | start_x);
    tail++;
    ...
    {
        unsigned char i;
        unsigned short packed = g_queue_xy[head];
        unsigned char x = (unsigned char)(packed & 0x00FFU);
        unsigned char y = (unsigned char)(packed >> 8);
        ...
        for(i = 0; i < 8; i++)
        {
            ...
            g_queue_xy[tail] = (unsigned short)(
                ((unsigned short)ny << 8) | (unsigned short)nx);
            tail++;
        }
    }
