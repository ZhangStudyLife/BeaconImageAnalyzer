...
#define GRAY_BEACON_SPARSE_MAX_BRIGHT_PIXELS \
    ((BEACON_IMAGE_W * BEACON_IMAGE_H) / 32)
    ...
    int shape_max_area)
{
    ...
    while(head < tail)
    {
        ...
        for(i = 0; i < 8; i++)
        {
            ...
            if((g_binary[ny][nx] == 0) || (tail >= IMAGE_QUEUE_SIZE))
            {
                continue;
            }
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    for(y = 2; y <= BEACON_IMAGE_H - 3; y++)
    {
        for(x = 2; x <= BEACON_IMAGE_W - 3; x++)
        {
            ...
            response = gray_integral_response(x, y);
            if((response <= 0) ||
            ...
            if((response < gray_integral_response(x - 1, y - 1)) ||
               (response < gray_integral_response(x - 1, y)) ||
               (response < gray_integral_response(x - 1, y + 1)) ||
               (response < gray_integral_response(x, y - 1)) ||
               (response < gray_integral_response(x, y + 1)) ||
               (response < gray_integral_response(x + 1, y - 1)) ||
               (response < gray_integral_response(x + 1, y)) ||
               (response < gray_integral_response(x + 1, y + 1)))
    ...
    const component_t *comp)
{
    ...
    for(y = comp->min_y; y <= comp->max_y; y++)
    {
        for(x = comp->min_x; x <= comp->max_x; x++)
        {
            ...
            gray = image[y][x];
            if(image[y][x] < CAR_LAMP_GRAY_THRESHOLD)
            {
                continue;
            }
            sum += gray;
