...
#define GRAY_BEACON_BRIGHT_WORD_COUNT  ((BEACON_IMAGE_W + 31) / 32)
...
static unsigned int g_gray_bright_bits[BEACON_IMAGE_H]
                                      [GRAY_BEACON_BRIGHT_WORD_COUNT];
...
static unsigned int gray_count_trailing_zeros(unsigned int value)
{
    ...
    gray_beacon_peak_t peaks[GRAY_BEACON_MAX_PEAKS])
{
    ...
    for(y = 2; y <= BEACON_IMAGE_H - 3; y++)
    {
        ...
        for(word_index = 0;
            word_index < GRAY_BEACON_BRIGHT_WORD_COUNT;
            word_index++)
        {
            unsigned int bright_bits = g_gray_bright_bits[y][word_index];

            while(bright_bits != 0U)
            {
                unsigned int bit = gray_count_trailing_zeros(bright_bits);
                int x = word_index * 32 + (int)bit;
                signed short response;

                bright_bits &= bright_bits - 1U;
                if((x < 2) || (x > BEACON_IMAGE_W - 3))
                {
                    continue;
                }
                ...
                if((response <
                    gray_cached_integral_response(x - 1, y - 1)) ||
                   (response <
                    gray_cached_integral_response(x - 1, y)) ||
                   (response <
                    gray_cached_integral_response(x - 1, y + 1)) ||
                   (response <
                    gray_cached_integral_response(x, y - 1)) ||
                   (response <
                    gray_cached_integral_response(x, y + 1)) ||
                   (response <
                    gray_cached_integral_response(x + 1, y - 1)) ||
    ...
    unsigned short *bright_pixel_count)
{
    ...
    memset(g_gray_bright_bits, 0, sizeof(g_gray_bright_bits));

#if IMAGE_USE_CM4_DSP
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        const unsigned char *pixel = image[y];
        int x;

        for(x = 0; x < BEACON_IMAGE_W; x += 4)
        {
            unsigned int pixels = *(const unsigned int *)&pixel[x];
            ...
            g_gray_bright_bits[y][x >> 5] |= bits << (x & 31);
        }
    }
#else
    for(y = 0; y < BEACON_IMAGE_H; y++)
    {
        ...
        for(x = 0; x < BEACON_IMAGE_W; x++)
        {
            ...
            sum += pixel;
            if(pixel >= GRAY_BEACON_MIN_PEAK_GRAY)
            {
                bright++;
                g_gray_bright_bits[y][x >> 5] |= 1UL << (x & 31);
            }
        }
    }
#endif
