...
static int cm4_dsp_emulate_smlad(
    unsigned int left,
    unsigned int right,
    int accumulator)
{
    int low = (int)(signed short)(left & 0xFFFFU) *
              (int)(signed short)(right & 0xFFFFU);
    int high = (int)(signed short)(left >> 16) *
               (int)(signed short)(right >> 16);

    return accumulator + low + high;
}
...
#define __SMLAD(left, right, accumulator) \
    cm4_dsp_emulate_smlad((left), (right), (accumulator))
