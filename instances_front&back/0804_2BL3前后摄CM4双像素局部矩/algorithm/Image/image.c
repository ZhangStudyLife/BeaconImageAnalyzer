...
#if IMAGE_USE_CM4_DSP
static inline void gray_integral_write_pair(
    ...
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W])
{
    ...
    for(padded_y = 0; padded_y < GRAY_BEACON_PADDED_H; padded_y++)
    {
        ...
        current[0] = 0U;
#if IMAGE_USE_CM4_DSP
        row_sum += source[GRAY_BEACON_BOX_PAD];
        current[1] = (unsigned short)(previous[1] + row_sum);
        output_x = 2;
        gray_integral_write_pair(
            previous, current, &output_x, &row_sum,
            source[3], source[2]);
        gray_integral_write_pair(
            previous, current, &output_x, &row_sum,
            source[1], source[0]);
        for(x = 1; x < BEACON_IMAGE_W - 1; x += 2)
        {
            gray_integral_write_pair(
                previous, current, &output_x, &row_sum,
                source[x], source[x + 1]);
    ...
    unsigned char point_source_mode)
{
    ...
    for(dy = -6; dy <= 6; dy++)
    {
        ...
        for(dx = -half_width; dx < half_width; dx += 2)
        {
            unsigned char pixel0 = core_pixels[dy + 6][dx + 6];
            unsigned char pixel1 = core_pixels[dy + 6][dx + 7];
            int weight0 = (int)pixel0 * 2 - background_twice;
            int weight1 = (int)pixel1 * 2 - background_twice;
            unsigned int weights;
            unsigned int coordinate_x;
            unsigned int coordinate_y;
            unsigned int coordinate_xx;
            unsigned int coordinate_yy;
            unsigned int coordinate_xy;
            unsigned int inner_mask;

            if(weight0 < 0) weight0 = 0;
            if(weight1 < 0) weight1 = 0;
            weights = (unsigned int)(unsigned short)(signed short)weight0 |
                      ((unsigned int)(unsigned short)(signed short)weight1 << 16);
            coordinate_x =
                (unsigned int)(unsigned short)(signed short)dx |
                ((unsigned int)(unsigned short)(signed short)(dx + 1) << 16);
            coordinate_y =
                (unsigned int)(unsigned short)(signed short)dy |
                ((unsigned int)(unsigned short)(signed short)dy << 16);
            coordinate_xx =
                (unsigned int)(unsigned short)(dx * dx) |
                ((unsigned int)(unsigned short)((dx + 1) * (dx + 1)) << 16);
            coordinate_yy =
                (unsigned int)(unsigned short)(dy * dy) |
                ((unsigned int)(unsigned short)(dy * dy) << 16);
            coordinate_xy =
                (unsigned int)(unsigned short)(signed short)(dx * dy) |
                ((unsigned int)(unsigned short)(signed short)((dx + 1) * dy) << 16);
            inner_mask =
                (unsigned int)(((dx * dx + dy * dy) <= 4) ? 1U : 0U) |
                (((unsigned int)((((dx + 1) * (dx + 1) + dy * dy) <= 4) ?
                                 1U : 0U)) << 16);

            weight_sum_twice = __SMLAD(
                weights, 0x00010001U, weight_sum_twice);
            weight_x_twice = __SMLAD(
                weights, coordinate_x, weight_x_twice);
            weight_y_twice = __SMLAD(
                weights, coordinate_y, weight_y_twice);
            weight_xx_twice = __SMLAD(
                weights, coordinate_xx, weight_xx_twice);
            weight_yy_twice = __SMLAD(
                weights, coordinate_yy, weight_yy_twice);
            weight_xy_twice = __SMLAD(
                weights, coordinate_xy, weight_xy_twice);
            inner_weight_twice = __SMLAD(
                weights, inner_mask, inner_weight_twice);
            if((int)pixel0 * 2 > (int)peak) half_area++;
            if((int)pixel1 * 2 > (int)peak) half_area++;
        }
        {
            unsigned char pixel = core_pixels[dy + 6][half_width + 6];
            int weight_twice = (int)pixel * 2 - background_twice;
            int radius2 = half_width * half_width + dy * dy;

            if(weight_twice < 0) weight_twice = 0;
            weight_sum_twice += weight_twice;
            weight_x_twice += weight_twice * half_width;
            weight_y_twice += weight_twice * dy;
            weight_xx_twice += weight_twice * half_width * half_width;
            weight_yy_twice += weight_twice * dy * dy;
            weight_xy_twice += weight_twice * half_width * dy;
            if(radius2 <= 4) inner_weight_twice += weight_twice;
            if((int)pixel * 2 > (int)peak) half_area++;
        }
    }
#else
    for(dy = -6; dy <= 6; dy++)
