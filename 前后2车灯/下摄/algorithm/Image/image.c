...
#define DOWN_GRAY_EDGE_MIN_PEAK         195U
...
#define DOWN_CAR_TINY_CORE_MAX_AREA     7
#define DOWN_CAR_TINY_ENVELOPE_MIN_AREA 18
...
#define DOWN_SHARED_CAR_ENVELOPE_ELONGATION 2.50f
#define DOWN_SHARED_BEACON_ELONGATION   2.40f
#define DOWN_EDGE_WEAK_MIN_CONTRAST     80.0f
#define DOWN_EDGE_WEAK_MIN_RADIAL_DROP  60.0f
#define DOWN_EDGE_WEAK_MIN_CONCENTRATION 0.50f
...
#define DOWN_BEACON_RULE_WEAK           0x01U
#define DOWN_BEACON_RULE_NORMAL         0x02U
#define DOWN_BEACON_RULE_MEDIUM         0x04U
#define DOWN_BEACON_RULE_LARGE          0x08U
#define DOWN_BEACON_RULE_COMPACT        (DOWN_BEACON_RULE_NORMAL | \
                                         DOWN_BEACON_RULE_MEDIUM | \
                                         DOWN_BEACON_RULE_LARGE)

#define DOWN_OBJECT_CLASS_NONE          0U
#define DOWN_OBJECT_CLASS_CAR           1U
#define DOWN_OBJECT_CLASS_BEACON        2U
#define DOWN_OBJECT_CLASS_BACKGROUND    3U
#define DOWN_OBJECT_CLASS_AMBIGUOUS     4U
...
typedef struct
{
    ...
    unsigned char background_rejected;
    ...
    down_car_features_t *features)
{
    ...
    /*
     * 单个极小饱和核心很容易由点状信标产生。只有其灰度包络本身
     * 已形成足够长的亮条时，才允许进入车灯分类；相邻断裂车灯会在
     * 连通域整理阶段先合并，不依赖这里放宽点状核心。
     */
    if (comp->area <= DOWN_CAR_TINY_CORE_MAX_AREA &&
        (features->envelope.area < DOWN_CAR_TINY_ENVELOPE_MIN_AREA ||
         features->envelope.elongation <
             DOWN_CAR_TINY_ENVELOPE_MIN_ELONGATION) &&
        !((features->envelope.min_x <= 0 ||
           features->envelope.min_y <= 0 ||
           features->envelope.max_x >= BEACON_IMAGE_W - 1 ||
           features->envelope.max_y >= BEACON_IMAGE_H - 1) &&
          features->envelope.elongation >= 2.60f))
    {
        return 0U;
    }
...
static int car_core_axis_gap(int first_min,
                             int first_max,
                             int second_min,
                             int second_max)
{
...
static component_t merge_car_core_pair(const component_t *left,
                                        const component_t *right);

static unsigned char car_cores_form_one_strip(const component_t *left,
                                               const component_t *right,
                                               component_t *combined)
{
    component_t probe;
    component_t envelope;
    int gap_x;
    ...
    probe = merge_car_core_pair(left, right);
    if (probe.valid == 0U)
    {
        return 0U;
    }
    if (probe.elongation < DOWN_CAR_CORE_MERGE_MIN_ELONGATION)
    {
        return 0U;
    }

    pad = car_envelope_pad(&probe);
    background = car_local_background_median(&probe, pad);
    delta = ((float)probe.peak - (float)background) * 0.22f;
    if (delta < 20.0f)
    {
        delta = 20.0f;
    }
    threshold = (int)((float)background + delta + 0.5f);
    if (threshold < 90) threshold = 90;
    if (threshold > 255) threshold = 255;
    if (car_find_core_seed(left, &left_seed_x, &left_seed_y) == 0U ||
        car_find_core_seed(right, &right_seed_x, &right_seed_y) == 0U)
    {
        return 0U;
    }
    envelope = grow_car_envelope(&probe, threshold, pad);
    if (envelope.valid == 0U ||
        is_visited(left_seed_x, left_seed_y) == 0U ||
        is_visited(right_seed_x, right_seed_y) == 0U)
    {
        /*
         * 绳索或其他遮挡物可能切断车灯的灰度路径。已有车灯轨迹、
...
{
    component_t merged;
    unsigned short coordinate_count = 0U;
    int sum_x = 0;
    int sum_y = 0;
    int min_x = (left->min_x < right->min_x) ? left->min_x : right->min_x;
    int min_y = (left->min_y < right->min_y) ? left->min_y : right->min_y;
    int max_x = (left->max_x > right->max_x) ? left->max_x : right->max_x;
    int max_y = (left->max_y > right->max_y) ? left->max_y : right->max_y;
    int x;
    int y;
    ...
    for (y = min_y; y <= max_y; y++)
    {
        ...
        {
            unsigned int gray;
            if (g_binary[y][x] == 0U || coordinate_count >= IMAGE_QUEUE_SIZE)
            {
                continue;
            }
            gray = g_current_image[y][x];
            g_queue[coordinate_count++] =
                (unsigned short)x | ((unsigned short)y << 8U);
            merged.area++;
            merged.gray_sum += gray;
            merged.gray_sum_sq += gray * gray;
            if (gray > merged.peak)
            {
                merged.peak = (unsigned char)gray;
            }
            sum_x += x;
            sum_y += y;
        }
    }
    finish_component(&merged, sum_x, sum_y, coordinate_count);
    if (merged.valid != 0U)
    {
        merged.merged_strip = 1U;
    }
    return merged;
}

static unsigned char merge_aligned_car_cores(
    component_t cores[DOWN_CAR_MAX_COMPONENTS],
    unsigned char core_count)
{
    unsigned char first = 0U;

    while (first < core_count)
    {
        unsigned char second = (unsigned char)(first + 1U);
        unsigned char merged = 0U;
        while (second < core_count)
        {
            component_t combined;
            if (car_cores_form_one_strip(
                    &cores[first], &cores[second], &combined) != 0U)
            {
                unsigned char move;
                if (combined.valid != 0U)
                {
                    cores[first] = combined;
                    for (move = second; move + 1U < core_count; move++)
                    {
                        cores[move] = cores[move + 1U];
                    }
                    core_count--;
                    merged = 1U;
                    break;
    ...
    const down_car_features_t *features)
{
    ...
    for (index = 0U; index < *candidate_count; index++)
    {
        ...
        if (features->score > candidates[index].score)
        {
            ...
            candidates[index].touches_border =
                (features->envelope.min_x <= 0 ||
                 features->envelope.min_y <= 0 ||
                 features->envelope.max_x >= BEACON_IMAGE_W - 1 ||
    ...
    if (*candidate_count < DOWN_CAR_MAX_MASKS)
    {
        ...
        candidates[*candidate_count].touches_border =
            (features->envelope.min_x <= 0 ||
             features->envelope.min_y <= 0 ||
             features->envelope.max_x >= BEACON_IMAGE_W - 1 ||
             features->envelope.max_y >= BEACON_IMAGE_H - 1) ? 1U : 0U;
...
static unsigned char car_candidate_better(
    const down_car_candidate_t *candidate,
    const down_car_candidate_t *current)
{
    if (candidate == 0 || candidate->object_class != DOWN_OBJECT_CLASS_CAR)
    {
        return 0U;
    }
    if (current == 0 || current->object_class != DOWN_OBJECT_CLASS_CAR)
    {
        return 1U;
    }
    if (candidate->touches_border != current->touches_border)
    {
        return (candidate->touches_border == 0U) ? 1U : 0U;
    }
    return (candidate->score > current->score) ? 1U : 0U;
}
    ...
    unsigned char classification)
{
    ...
    for (index = 0U; index < candidate_count; index++)
    {
        if (candidates[index].classification != classification ||
            candidates[index].object_class != DOWN_OBJECT_CLASS_CAR)
        {
            continue;
        }
    ...
    if (tracked < candidate_count &&
        candidates[tracked].touches_border == candidates[best].touches_border &&
...
#pragma inline=never
#endif
static unsigned char collect_car_candidates(
    down_car_candidate_t candidates[DOWN_CAR_MAX_MASKS])
{
    component_t cores[DOWN_CAR_MAX_COMPONENTS];
    ...
    memset(candidates, 0,
           sizeof(down_car_candidate_t) * DOWN_CAR_MAX_MASKS);
    g_current_lamp_mask_count = 0U;
    begin_visit_pass();
    ...
    core_count = merge_aligned_car_cores(cores, core_count);
...
static unsigned char select_current_car_lamp(
    const down_car_candidate_t candidates[DOWN_CAR_MAX_MASKS],
    unsigned char candidate_count,
...
static unsigned char down_gray_point_in_range(int x, int y, float clearance)
{
    float top_y;
    float bottom_y;
    ...
    if (top_y >= 0.0f && top_y <= (float)(BEACON_IMAGE_H - 1) &&
        (float)y < top_y + clearance)
    {
        return 0U;
    }
    if (bottom_y >= 0.0f && bottom_y <= (float)(BEACON_IMAGE_H - 1) &&
        (float)y > bottom_y - clearance)
    {
        return 0U;
    }
    return 1U;
...
static unsigned char down_gray_candidate_rules(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    int x,
    int y,
    float scene_mean,
    ...
    unsigned char *background_rejected)
{
    ...
    if (background_rejected != 0)
    {
        *background_rejected = 0U;
    }

    if (features == 0 || rule_mask == 0 || background_rejected == 0 ||
        down_gray_point_in_range(
            x, y, DOWN_BEACON_BOUNDARY_CLEARANCE) == 0U)
    {
        return 0U;
    }
    if (weak_peak < 140)
    ...
    rules = (weak != 0U ? DOWN_BEACON_RULE_WEAK : 0U) |
            (normal != 0U ? DOWN_BEACON_RULE_NORMAL : 0U) |
            (medium != 0U ? DOWN_BEACON_RULE_MEDIUM : 0U) |
            (large != 0U ? DOWN_BEACON_RULE_LARGE : 0U);
    if (rules == 0U)
    {
        return 0U;
    }
    ...
    is_edge = (x < DOWN_BEACON_LEFT_EDGE_MARGIN ||
               x >= BEACON_IMAGE_W - DOWN_BEACON_RIGHT_EDGE_MARGIN) ? 1U : 0U;
    if (is_edge != 0U &&
        (features->peak < DOWN_GRAY_EDGE_MIN_PEAK ||
         (features->outer_occupancy > DOWN_GRAY_EDGE_MAX_OCCUPANCY &&
          (features->half_area > 20U ||
           features->concentration < 0.52f))))
    {
        *background_rejected = 1U;
    }
    if (is_edge != 0U &&
        ...
        !(large != 0U &&
          features->peak >= 248U &&
          features->outer_occupancy <= 0.10f))
    {
        *background_rejected = 1U;
    }
    if (is_edge != 0U && rules == DOWN_BEACON_RULE_WEAK &&
        (features->inner_contrast < DOWN_EDGE_WEAK_MIN_CONTRAST ||
         features->radial_drop < DOWN_EDGE_WEAK_MIN_RADIAL_DROP ||
         features->concentration < DOWN_EDGE_WEAK_MIN_CONCENTRATION))
    {
        *background_rejected = 1U;
    }
    *rule_mask = rules;
...
static unsigned char collect_beacon_candidates(
    const unsigned char image[BEACON_IMAGE_H][BEACON_IMAGE_W],
    float scene_mean,
    down_gray_candidate_t candidates[DOWN_GRAY_MAX_CANDIDATES])
{
    ...
    memset(candidates, 0,
           sizeof(down_gray_candidate_t) * DOWN_GRAY_MAX_CANDIDATES);
    peak_count = down_gray_find_peaks(
        image, 0, 0, scene_mean, peaks);
    for (index = 0U; index < peak_count; index++)
    {
        ...
        if (candidate.x < 0.0f || candidate.x > (float)(BEACON_IMAGE_W - 1) ||
            ...
            down_gray_candidate_rules(
                image, (int)(candidate.x + 0.5f),
                (int)(candidate.y + 0.5f), scene_mean, &features,
                peak_clipped, &rule_mask, &background_rejected) == 0U)
        {
            continue;
        }
        ...
        candidate.rule_mask = rule_mask;
        candidate.background_rejected = background_rejected;
        candidate.object_class = DOWN_OBJECT_CLASS_NONE;
        down_gray_add_candidate(candidates, &candidate_count, &candidate);
    }
    return candidate_count;
}

static unsigned char classify_shared_pair(
    const down_car_candidate_t *car,
    const down_gray_candidate_t *beacon)
{
    if (beacon->background_rejected != 0U)
    {
        return DOWN_OBJECT_CLASS_BACKGROUND;
    }
    /* weak-only响应不足以和已经成立的车灯证据竞争，只用于无车灯证据时的点光源输出。 */
    if ((beacon->rule_mask & DOWN_BEACON_RULE_COMPACT) == 0U)
    {
        return DOWN_OBJECT_CLASS_CAR;
    }
    if (car->component.merged_strip != 0U ||
        (car->component.elongation >= DOWN_SHARED_CAR_ELONGATION &&
         car->mask.elongation >= DOWN_SHARED_CAR_ENVELOPE_ELONGATION) ||
        (car->touches_border != 0U &&
         car->component.elongation >= DOWN_SHARED_CAR_ELONGATION))
    {
        return DOWN_OBJECT_CLASS_CAR;
    }
    if (car->component.elongation < DOWN_SHARED_BEACON_ELONGATION &&
        car->mask.elongation < DOWN_SHARED_BEACON_ELONGATION &&
        (beacon->rule_mask & DOWN_BEACON_RULE_COMPACT) != 0U)
    {
        return DOWN_OBJECT_CLASS_BEACON;
    }
    return DOWN_OBJECT_CLASS_AMBIGUOUS;
}

static void classify_shared_candidates(
    ...
    unsigned char beacon_count)
{
    unsigned char beacon_matched[DOWN_GRAY_MAX_CANDIDATES];
    unsigned char car_index;
    unsigned char beacon_index;

    memset(beacon_matched, 0, sizeof(beacon_matched));
    for (car_index = 0U; car_index < car_count; car_index++)
    {
        car_candidates[car_index].object_class = DOWN_OBJECT_CLASS_CAR;
    }
    for (beacon_index = 0U; beacon_index < beacon_count; beacon_index++)
    {
        beacon_candidates[beacon_index].object_class =
            (beacon_candidates[beacon_index].background_rejected != 0U) ?
                DOWN_OBJECT_CLASS_BACKGROUND : DOWN_OBJECT_CLASS_BEACON;
    }

    for (car_index = 0U; car_index < car_count; car_index++)
    {
        unsigned char nearest = 255U;
        float nearest_distance = DOWN_SHARED_CANDIDATE_DISTANCE_SQ;
        for (beacon_index = 0U; beacon_index < beacon_count; beacon_index++)
        {
            float dx;
            float dy;
            float distance;
            if (beacon_matched[beacon_index] != 0U)
            {
                continue;
            }
            dx = car_candidates[car_index].component.cx -
                 beacon_candidates[beacon_index].x;
            dy = car_candidates[car_index].component.cy -
                 beacon_candidates[beacon_index].y;
            distance = dx * dx + dy * dy;
            if (distance <= nearest_distance)
            {
                nearest_distance = distance;
                nearest = beacon_index;
            }
        }
        if (nearest < beacon_count)
        {
            unsigned char object_class = classify_shared_pair(
                &car_candidates[car_index], &beacon_candidates[nearest]);
            car_candidates[car_index].object_class = object_class;
            beacon_candidates[nearest].object_class = object_class;
            beacon_matched[nearest] = 1U;
        }
    }
}
    ...
    const component_t *temporal_lamp)
{
    ...
    for (index = 0U; index < candidate_count; index++)
    {
        ...
        {
            add_car_mask((candidates[index].component.merged_strip != 0U) ?
                             &candidates[index].component :
    ...
    beacon_result_t *result)
{
    ...
    for (index = 0U; index < candidate_count; index++)
    {
        int x;
        int y;
        if (candidates[index].object_class != DOWN_OBJECT_CLASS_BEACON)
        {
            continue;
        }
        x = (int)(candidates[index].x + 0.5f);
        y = (int)(candidates[index].y + 0.5f);
...
static void remove_beacons_covered_by_final_car(beacon_result_t *result)
{
    ...
    for (read_index = 0U; read_index < result->beacon_count; read_index++)
    {
        ...
        {
            result->beacons[write_index] = result->beacons[read_index];
        }
        write_index++;
    }
    result->beacon_count = write_index;
    sync_legacy_beacons(result);
}
    ...
    beacon_result_t *result)
{
    ...
    car_candidate_count = collect_car_candidates(car_candidates);
    beacon_candidate_count = collect_beacon_candidates(
        image, scene_mean, beacon_candidates);
