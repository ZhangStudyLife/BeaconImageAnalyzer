...
static float s_pixel_radial[IMAGE_HORIZON_WIDTH][IMAGE_HORIZON_HEIGHT];
static float s_pixel_ray_z[IMAGE_HORIZON_WIDTH][IMAGE_HORIZON_HEIGHT];
...
static void image_horizon_lookup(float radius2, float *radial, float *ray_z)
{
...
static inline float image_horizon_value(uint16 x, uint16 y)
{
    float radial = s_pixel_radial[x][y];

    return s_normalized_x[x] * radial * s_current_normal[0] +
           s_normalized_y[y] * radial * s_current_normal[1] +
           s_pixel_ray_z[x][y] * s_current_normal[2];
}

static uint8 image_horizon_root_in_interval(uint16 y,
                                            float first,
                                            float second,
                                            float *root)
{
                                     ...
                                     float *root)
{
    ...
    lower_second = image_horizon_value(x, (uint16)center);
    upper_first = lower_second;
    for(distance = 0; distance < (int)IMAGE_HORIZON_HEIGHT; distance++)
    {
        ...
        if(lower >= 0)
        {
            float lower_first = image_horizon_value(x, (uint16)lower);
            lower_valid = image_horizon_root_in_interval(
                (uint16)lower, lower_first, lower_second, &lower_root);
            lower_second = lower_first;
        }
        if(upper < ((int)IMAGE_HORIZON_HEIGHT - 1))
        {
            float upper_second =
                image_horizon_value(x, (uint16)(upper + 1));
            upper_valid = image_horizon_root_in_interval(
                (uint16)upper, upper_first, upper_second, &upper_root);
...
static uint8 image_horizon_find_seed(uint16 *seed_x, float *seed_y)
{
...
void image_horizon_init(void)
{
    ...
    {
        for(y = 0U; y < IMAGE_HORIZON_HEIGHT; y++)
        {
            float radius2 =
                s_normalized_x[x] * s_normalized_x[x] +
                s_normalized_y[y] * s_normalized_y[y];

            image_horizon_lookup(radius2,
                                 &s_pixel_radial[x][y],
                                 &s_pixel_ray_z[x][y]);
        }
    }
                          ...
                          uint8 height_valid)
{
    ...
    for(row = 0U; row < 3U; row++)
    {
        ...
        s_current_normal[row] = normal[row];
    }

    memset(g_image_horizon_column_valid, 0, sizeof(g_image_horizon_column_valid));
    if(image_horizon_find_seed(&seed_x, &seed_y) == 0U)
    {
        g_image_horizon_valid = 0U;
        return;
    }
    ...
    {
        float root;
        if(image_horizon_find_root((uint16)x, previous_y, &root) == 0U)
        {
            break;
        }
