                    ...
                    const QString& groupId)
{
    stream << frame << ','
           ...
           << ",,," << ','  // width, length, angle 留空
           << groupId << '\n';
}

void writeCsvRect(QTextStream& stream,
                  int frame,
                  double timeSec,
                  const QString& targetType,
                  int index,
                                    ...
                                    QString* errorMessage) const
{
    ...
    stream << "frame,time_sec,target_type,index,valid,x,y,radius,width,length,angle,group_id\n";
    ...
    for (int frame = 0; frame < reader.frameCount(); ++frame)
    {
        ...
        for (int i = 0; i < carLampCount; ++i)
        {
            ...
            writeCsvRect(stream, frame, timeSec, QStringLiteral("car_lamp"), i, rect, QStringLiteral("car0"));
        }
